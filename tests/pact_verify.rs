...
use pact_verifier::{
    FilterInfo, NullRequestFilterExecutor, PactSource, ProviderInfo, PublishOptions,
    VerificationOptions, verify_provider_async,
};
use prost::Message;
    ...
    async fn from_data(req: &'r Request<'_>, data: Data<'r>) -> Outcome<'r, Self> {
        if let Some(content_type) = req.content_type() {
            if content_type.is_json() {
                        ...
                        Ok(json) => {
                            if let Some(contents) = json.get("request") {
                                ...
                                match MessageContents::from_json(contents) {
                                    ...
                                    Err(err) => {
                                        Outcome::Error((Status::UnprocessableEntity, anyhow!(err)))
                                    }
                                }
                            } else {
                                Outcome::Error((
                                    Status::UnprocessableEntity,
                                    anyhow!("Missing request"),
                                ))
                            }
                        }
                        Err(err) => Outcome::Error((
                            Status::UnprocessableEntity,
                            anyhow!("Failed to parse JSON: {}", err),
                        )),
                    },
                ...
                Outcome::Error((
                    Status::UnprocessableEntity,
                    anyhow!("Expected JSON, got {}", content_type),
                ))
            }
        } else {
...
async fn messages(request: MessageRequest) -> (Status, (ContentType, Vec<u8>)) {
    ...
    if let Some(content_type) = request.contents.message_content_type() {
        if content_type.sub_type == "protobuf" {
            if let Some(message_type) = content_type.attributes.get("message") {
                match message_type.as_str() {
                    ...
                    _ => (
                        ...
                        (
                            ContentType::Text,
                            Vec::from("Unknown protobuf message type provided"),
                        ),
                    ),
                }
            } else {
                (
                    ...
                    (
                        ContentType::Text,
                        Vec::from("Not a protobuf message type provided"),
                    ),
                )
            }
        } else {
            (
                Status::BadRequest,
                (ContentType::Text, Vec::from("Not a protobuf message")),
            )
        }
    } else {
        (
            ...
            (
                ContentType::Text,
                Vec::from("No content type for the provided message"),
            ),
        )
    }
}
...
async fn init_plugin_request(request_message: &MessageRequest) -> (Status, (ContentType, Vec<u8>)) {
    if let Some(data) = request_message.contents.contents.value() {
        match InitPluginRequest::decode(data) {
            Ok(request) => {
                ...
                match plugin.init_plugin(tonic::Request::new(request)).await {
                    Ok(response) => {
                        ...
                        match response.get_ref().encode(&mut buffer) {
                            Ok(_) => (
                                Status::Ok,
                                (
                                    ContentType::new("application", "protobuf").with_params((
                                        "message",
                                        ".io.pact.plugin.InitPluginResponse",
                                    )),
                                    buffer.to_vec(),
                                ),
                            ),
                            Err(err) => {
                                ...
                                (
                                    ...
                                    (
                                        ContentType::Text,
                                        Vec::from("Failed to write response to buffer"),
                                    ),
                                )
                            }
                        }
                    ...
                    Err(err) => {
                        error!(
                            "Failed to generate response for InitPluginRequest - {}",
                            err
                        );
                        (
                            Status::BadRequest,
                            (
                                ContentType::Text,
                                Vec::from("Failed to generate response message"),
                            ),
                        )
                    }
                }
            ...
            Err(err) => {
                ...
                (
                    ...
                    (
                        ContentType::Text,
                        Vec::from("Failed to parse request message"),
                    ),
                )
            }
        }
        ...
        (
            ...
            (
                ContentType::Text,
                Vec::from("Request did not contain any data"),
            ),
        )
    }
}
...
async fn verify_plugin() {
    ...
    let source = PactSource::BrokerWithDynamicConfiguration {
        ...
        selectors: vec![],
        auth: Some(HttpAuth::Token(
            env::var("PACTFLOW_TOKEN")
                .expect("The PACTFLOW_TOKEN environment variable must be set")
                .to_string(),
        )),
        links: vec![],
    };
    ...
    let server = rocket::build()
        ...
        .ignite()
        .await
        .expect("Could not start the Rocket server");
