...
use pact_protobuf_plugin::message_decoder::ProtobufFieldData::{
    Boolean, Double, Integer32, String, UInteger32,
};
use pact_protobuf_plugin::message_decoder::{ProtobufField, decode_message};
use pact_protobuf_plugin::utils::{
    find_message_descriptor_for_type, get_descriptors_for_interaction, lookup_interaction_config,
};
...
async fn basic_values_test() {
    ...
    let (message_descriptor, _) =
        find_message_descriptor_for_type(".com.pact.protobuf.example.MessageIn", &fds).unwrap();
    ...
    let field_descriptor3 = message_descriptor
        ...
        .unwrap();
    let field_descriptor4 = message_descriptor
        .field
        .iter()
        .find(|field| field.number == Some(4))
        .unwrap();
    let field_descriptor5 = message_descriptor
        .field
        .iter()
