...
mod tests {
    ...
    async fn test_proto_client() {
        ...
        let mut client = feature_service_client::FeatureServiceClient::connect(url.to_string())
            .await
            .unwrap();

        let request_message = Point { x: 11.0, y: 22.0 };
        let response = client
            .get_feature(tonic::Request::new(request_message))
            .await;
