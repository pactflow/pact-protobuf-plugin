...
mod tests {
    ...
    use super::*;
    use pact_models::{
        matchingrules::{Category, MatchingRule, MatchingRules, RuleLogic},
        path_exp::DocPath,
    };
    use serde_json::json;
    ...
    async fn test_proto_client() {
        ...
        expect!(interaction.response[0].matching_rules.clone())
            .to(be_equal_to(expected_response_rules));

        let mock_server = builder_async
            .start_mock_server_async(Some("protobuf/transport/grpc"), None)
            .await;
        let url = mock_server.url();
        let mut client = test_client::TestClient::connect(url.to_string())
            .await
            .unwrap();
        ...
        request
            .metadata_mut()
            .insert("key", tonic::metadata::MetadataValue::from_static("value"));
