...
use pact_models::matchingrules::expressions::{
    MatchingRuleDefinition, ValueType, is_matcher_def, parse_matcher_def,
};
...
use prost_types::{
    DescriptorProto, FieldDescriptorProto, FileDescriptorProto, ServiceDescriptorProto, Struct,
};
...
use crate::utils::{
    find_enum_value_by_name, find_enum_value_by_name_in_message,
    find_message_descriptor_for_type_in_map, find_nested_type, is_map_field, is_repeated_field,
    last_name, prost_string, split_service_and_method, to_fully_qualified_name,
};
...
pub(crate) async fn process_proto(
    ...
    debug!(
        "Parsed proto file OK, file descriptors = {:?}",
        descriptors
            .file
            .iter()
    ...
    let file_descriptors: HashMap<String, &FileDescriptorProto> = descriptors
        .file
        .iter()
    ...
    let descriptor = match file_descriptors.get(file_name) {
        None => {
            return Err(anyhow!(
                "Did not find a file proto descriptor for the provided proto file '{}'",
                file_name
            ));
        }
        Some(des) => *des,
    ...
    if let Some(message_type) = config.get("pact:message-type") {
        let message = proto_value_to_string(message_type).ok_or_else(|| {
            anyhow!("Did not get a valid value for 'pact:message-type'. It should be a string")
        })?;
        debug!("Configuring a Protobuf message {}", message);
        let result = configure_protobuf_message(
            message.as_str(),
            config,
            descriptor,
        ...
        let service_name = proto_value_to_string(service_name).ok_or_else(|| {
            anyhow!("Did not get a valid value for 'pact:proto-service'. It should be a string")
        })?;
        debug!("Configuring a Protobuf service {}", service_name);
        let (request_part, response_part) = configure_protobuf_service(
            service_name.as_str(),
            config,
            descriptor,
            &file_descriptors,
...
fn configure_protobuf_service(
    ...
    debug!(
        "Looking for service and method with name '{}'",
        service_with_method
    );
    ...
    let service_descriptor = descriptor
        ...
        .ok_or_else(|| {
            anyhow!(
                "Did not find a descriptor for service '{}'",
                service_with_method
            )
        })?;
    trace!("service_descriptor = {:?}", service_descriptor);

    let service_with_method = service_with_method
        .split_once(':')
        .map(|(s, _)| s)
        .unwrap_or(service_with_method);
    ...
    .map(|(request, response)| {
        ...
        (
            request.map(|r| InteractionResponse {
                plugin_configuration: plugin_configuration.clone(),
                ..r
            }),
            response
                .iter()
                .map(|r| InteractionResponse {
                    plugin_configuration: plugin_configuration.clone(),
                    ..r.clone()
                })
                .collect(),
...
fn construct_protobuf_interaction_for_service(
    ...
    let service_name = service_descriptor
        .name
        .as_deref()
        .expect("Service descriptor name cannot be empty");
    trace!(
        ">> construct_protobuf_interaction_for_service({config:?}, {service_name}, {method_name})"
    );
    ...
    let method_descriptor = service_descriptor
        ...
        .ok_or_else(|| {
            anyhow!(
                "Did not find a method descriptor for method '{}' in service '{}'",
                method_name,
                service_name
            )
        })?;

    let input_name = method_descriptor.input_type.as_ref().ok_or_else(|| {
        anyhow!(
            "Input message name is empty for service {}/{}",
            service_name,
            method_name
        )
    })?;
    let output_name = method_descriptor.output_type.as_ref().ok_or_else(|| {
        anyhow!(
            "Output message name is empty for service {}/{}",
            service_name,
            method_name
        )
    })?;
    ...
    for (config, md_config) in response_part_config {
        ...
        let interaction = construct_protobuf_interaction_for_message(
            ...
            &config,
            "",
            &response_file_descriptor,
            all_descriptors,
            response_metadata.as_ref(),
        )?;
        response_part.push(InteractionResponse {
            part_name: "response".into(),
            ..interaction
        });
    }
...
fn response_part<'a>(
    ...
    Vec<(
        BTreeMap<String, prost_types::Value>,
                ...
                Kind::ListValue(l) => l
                    ...
                    .filter_map(|v| {
                        v.kind.as_ref().and_then(|k| match k {
                            ...
                            Kind::StringValue(_) => {
                                Some((btreemap! { "value".to_string() => v.clone() }, None))
                            }
                            _ => None,
                        })
                    })
                    .collect(),
                Kind::StringValue(_) => vec![(
                    btreemap! { "value".to_string() => response_config.clone() },
                    None,
                )],
                _ => vec![],
            })
...
fn configure_protobuf_message(
    ...
    trace!(
        ">> configure_protobuf_message({}, {:?})",
        message_name, descriptor_hash
    );
    debug!(
        "Looking for message '{}' in '{}'",
        message_name,
        descriptor.name()
    );
    let message_descriptor = descriptor
        ...
        .ok_or_else(|| {
            anyhow!(
                "Did not find a descriptor for message '{}' in '{}'",
                message_name,
                descriptor.name()
            )
        })?;
    ...
    .map(|interaction| InteractionResponse {
...
fn construct_protobuf_interaction_for_message(
    ...
    let message_name = message_descriptor
        .name
        .as_ref()
        .expect("Message descriptor name cannot be empty");
    let mut message_builder =
        MessageBuilder::new(message_descriptor, message_name, file_descriptor);
    ...
    Ok(InteractionResponse {
        ...
        metadata_rules: metadata
            .map(|md| extract_rules(&md.matching_rules))
            .unwrap_or_default(),
        metadata_generators: metadata
            .map(|md| extract_generators(&md.generators))
            .unwrap_or_default(),
        plugin_configuration: Some(PluginConfiguration {
...
) -> HashMap<String, pact_plugin_driver::proto::Generator> {
    generators
        .iter()
    ...
    if !field_name.starts_with("pact:") {
        if let Some(field) = message_builder.field_by_name(field_name) {
            ...
            match field.r#type {
                ...
                None => {
                    return Err(anyhow!(
                        "Message {} field '{}' is of an unknown type",
                        message_builder.message_name,
                        field_name
                    ));
                }
            }
            ...
            error!(
                "Field '{}' was not found in message '{}'",
                field_name, message_builder.message_name
            );
            let fields: HashSet<String> = message_builder
                .descriptor
                .field
                .iter()
                ...
                .collect();
            return Err(anyhow!(
                "Message {} has no field '{}'. Fields are {:?}",
                message_builder.message_name,
                field_name,
                fields
            ));
...
fn build_embedded_message_field_value(
    ...
    if is_repeated_field(field_descriptor)
        && !is_map_field(&message_builder.descriptor, field_descriptor)
    {
        ...
        match value {
                ...
                if let Some((first, rest)) = list.split_first() {
                    ...
                    for (index, item) in rest.iter().enumerate() {
                        ...
                        if let Some(constructed) = constructed {
                            message_builder.add_repeated_field_value(
                                field_descriptor,
                                field,
                                constructed,
                            );
            ...
            Value::Object(map) => {
                if let Some(definition) = map.get("pact:match") {
                    ...
                    debug!(
                        "Configuring repeated field from a matcher definition expression '{}'",
                        definition
                    );
                    ...
                    if let Some(each_value_def) = &each_value {
                        ...
                        if mrd.rules.len() > 1 {
                            warn!(
                                "{}: each value matcher can not be combined with other matchers, ignoring the other matching rules",
                                path
                            );
                        }
                        ...
                        match each_value_def.rules.first() {
                            Some(either) => match either {
                                Either::Left(_) => {
                                    matching_rules.add_rule(
                                        path.clone(),
                                        matchingrules::MatchingRule::EachValue(
                                            each_value_def.clone(),
                                        ),
                                        RuleLogic::And,
                                    );
                                    ...
                                    let constructed_value = value_for_type(
                                        field,
                                        each_value_def.value.as_str(),
                                        field_descriptor,
                                        &message_builder.descriptor,
                                        all_descriptors,
                                    )?;
                                    message_builder.set_field_value(
                                        field_descriptor,
                                        field,
                                        constructed_value,
                                    );
                                ...
                                Either::Right(reference) => {
                                    if let Some(field_value) = map.get(reference.name.as_str()) {
                                        matching_rules.add_rule(
                                            path.clone(),
                                            matchingrules::MatchingRule::Values,
                                            RuleLogic::And,
                                        );
                                        ...
                                        Err(anyhow!(
                                            "Expression '{}' refers to non-existent item '{}'",
                                            definition,
                                            reference.name
                                        ))
                                    }
                            ...
                            None => Err(anyhow!(
                                "Got an EachValue matcher with no associated matching rules to apply"
                            )),
                        }
                    } else {
                        if !mrd.rules.is_empty() {
                            for rule in &mrd.rules {
                                match rule {
                                    Either::Left(rule) => matching_rules.add_rule(
                                        path.clone(),
                                        rule.clone(),
                                        RuleLogic::And,
                                    ),
                                    Either::Right(mr) => {
                                        return Err(anyhow!(
                                            "References can only be used with an EachValue matcher - {:?}",
                                            mr
                                        ));
                                    }
                                }
                        ...
                        let constructed = value_for_type(
                            field,
                            mrd.value.as_str(),
                            field_descriptor,
                            &message_builder.descriptor,
                    ...
                    // single example.
                    trace!(
                        "No matching definition, assuming config contains the attributes of a single example"
                    );
                    build_single_embedded_field_value(
                        &path.join("*"),
                        message_builder,
...
fn build_single_embedded_field_value(
    ...
    debug!(
        "Configuring message field '{}' (type {:?})",
        field, field_descriptor.type_name
    );
    ...
    match type_name.as_str() {
        ...
        _ => {
            if is_map_field(&message_builder.descriptor, field_descriptor) {
        ...
        .ok_or_else(|| anyhow!("Did not find message '{}' in the current message or in the file descriptors", type_name))?;
                let mut embedded_builder = MessageBuilder::new(
                    &embedded_type,
                    last_name(type_name.as_str()),
                    &message_builder.file_descriptor,
                );
                ...
                let field_value = if let Some(definition) = config.get("pact:match") {
                    ...
                    for rule in &mrd.rules {
                        match rule {
                            ...
                            Either::Right(reference) => {
                                if let Some(field_def) = config.get(reference.name.as_str()) {
                                    matching_rules.add_rule(
                                        path.clone(),
                                        matchingrules::MatchingRule::Values,
                                        RuleLogic::And,
                                    ...
                                    field_value = build_single_embedded_field_value(
                                        &array_path,
                                        message_builder,
                                        MessageFieldValueType::Normal,
                                        field_descriptor,
                ...
                Err(anyhow!(
                    "For message fields, you need to define a Map of expected fields, got {:?}",
                    value
                ))
            }
        }
...
fn build_struct_field(
    ...
    trace!(
        ">> build_struct_field('{}', {}, {:?}, {:?}, {:?}, {:?})",
        path, field_name, field_value, message_builder, matching_rules, generators
    );
    ...
    match field_value {
        Value::Object(map) => {
            ...
            for (key, value) in map {
                let field_path = path.join(key);
                let proto_value =
                    build_proto_value(&field_path, value, matching_rules, generators)?;
            ...
            match field_type {
                MessageFieldValueType::Repeated => message_builder.add_repeated_field_value(
                    field_descriptor,
                    field_name,
                    message_field_value.clone(),
                ),
                _ => message_builder.set_field_value(
                    field_descriptor,
                    field_name,
                    message_field_value.clone(),
                ),
        ...
        _ => Err(anyhow!(
            "google.protobuf.Struct fields need to be configured with a Map, got {:?}",
            field_value
        )),
    }
}
...
fn build_proto_value(
    ...
    trace!(
        ">> build_proto_value('{}', {:?}, {:?}, {:?})",
        path, value, matching_rules, generators
    );
    match value {
        Value::Null => Ok(prost_types::Value {
            kind: Some(prost_types::value::Kind::NullValue(0)),
        }),
        Value::Bool(b) => Ok(prost_types::Value {
        ...
        Value::String(s) => {
            if is_matcher_def(s.as_str()) {
                ...
                if !mrd.rules.is_empty() {
                    for rule in &mrd.rules {
                        match rule {
                            ...
                            Either::Right(mr) => {
                                return Err(anyhow!(
                                    "Was expecting a value, but got a matching reference {:?}",
                                    mr
                                ));
                            }
                        }
                ...
                match mrd.value_type {
                    ValueType::Unknown | ValueType::String => Ok(prost_types::Value {
                        kind: Some(prost_types::value::Kind::StringValue(mrd.value.clone())),
                    }),
                    ...
                    ValueType::Integer => {
                        ...
                        Ok(prost_types::Value {
                            kind: Some(prost_types::value::Kind::NumberValue(num as f64)),
                        })
                    }
                    ValueType::Boolean => {
...
fn build_map_field(
    ...
    trace!(
        ">> build_map_field('{}', {}, {:?}, {:?})",
        path, field, value, message_builder
    );
    ...
    if let Value::Object(config) = value {
            ...
            if !mrd.rules.is_empty() {
                ...
                for rule in &mrd.rules {
                    match rule {
                        ...
                        Either::Right(mr) => {
                            return Err(anyhow!(
                                "Was expecting a matching rule definition, but got a reference: {}",
                                mr.name
                            ));
                        }
                    }
        ...
        if let Some(map_type) = find_nested_type(&message_builder.descriptor, field_descriptor) {
            ...
            let key_descriptor = map_type
                .field
                .iter()
                .find(|f| f.name.clone().unwrap_or_default() == "key")
                .ok_or_else(|| {
                    anyhow!("Did not find the key field in the descriptor for the map field")
                })?;
            let value_descriptor = map_type
                .field
                .iter()
                .find(|f| f.name.clone().unwrap_or_default() == "value")
                .ok_or_else(|| {
                    anyhow!("Did not find the value field in the descriptor for the map field")
                })?;
            ...
            for (inner_field, value) in config {
                if inner_field != "pact:match" {
                    ...
                    let key_value = build_field_value(
                        &entry_path,
                        &mut embedded_builder,
                        MessageFieldValueType::Normal,
                        key_descriptor,
                        "key",
                        &Value::String(inner_field.clone()),
                        matching_rules,
                        generators,
                        all_descriptors,
                    )?
                    .ok_or_else(|| {
                        anyhow!(
                            "Was not able to construct map key value {:?}",
                    ...
                    .ok_or_else(|| {
                        anyhow!(
                            "Was not able to construct map value value {:?}",
                            value_descriptor.type_name
                        )
                    })?;

                    message_builder.add_map_field_value(
                        field_descriptor,
                        field,
                        key_value,
                        value_value,
            ...
            Err(anyhow!(
                "Did not find the nested map type {:?} in the message descriptor nested types",
                field_descriptor.type_name
            ))
        }
    } else {
        Err(anyhow!(
            "Map fields need to be configured with a Map, got {:?}",
            value
        ))
    }
}
    ...
    match value {
        ...
        Value::String(s) => {
            let constructed_value = match field_type {
                MessageFieldValueType::Repeated => {
                    let path = path.join("*");
                    let constructed_value = construct_value_from_string(
                        &path,
                        message_builder,
                        descriptor,
                        field_name,
                    ...
                    debug!(
                        "Setting field {:?}:repeated to value {:?}",
                        field_name, constructed_value
                    );
                    message_builder.add_repeated_field_value(
                        descriptor,
                        field_name,
                        constructed_value.clone(),
                    );
                    constructed_value
                }
                _ => {
                    ...
                    debug!(
                        ...
                        field_name, field_type, constructed_value
                    );
                    message_builder.set_field_value(
                        descriptor,
                        field_name,
        ...
        Value::Array(list) => {
            if let Some((first, rest)) = list.split_first() {
                let index_path = path.join("0");
                let constructed_value = build_field_value(
                    &index_path,
                    message_builder,
                    MessageFieldValueType::Repeated,
                    descriptor,
        ...
        Value::Bool(b) => {
            if descriptor.r#type() == Type::Bool {
                ...
                Err(anyhow!(
                    "Only boolean field values can be configured with a boolean value, field {} type is {:?}",
                    field_name,
                    descriptor.r#type()
                ))
            }
        ...
        _ => Err(anyhow!(
            "Field values must be configured with a string value, got {:?}",
            value
        )),
    }
}
    ...
    match descriptor.r#type() {
        Type::Double => {
            if let Some(f) = f.to_f64() {
                ...
                Err(anyhow!(
                    "Can not construct a double value from the given value, field {} type is {:?} but value is {:?}",
                    field_name,
                    descriptor.r#type(),
                    value
                ))
        ...
        Type::Float => {
            if let Some(f) = f.to_f32() {
                ...
                Err(anyhow!(
                    "Can not construct a float value from the given value, field {} type is {:?} but value is {:?}",
                    field_name,
                    descriptor.r#type(),
                    value
                ))
        ...
        Type::Int32 | Type::Sint32 | Type::Sfixed32 => {
            if let Some(i) = f.to_i32() {
                ...
                Err(anyhow!(
                    "Can not construct an integer value from the given value, field {} type is {:?} but value is {:?}",
                    field_name,
                    descriptor.r#type(),
                    value
                ))
        ...
        Type::Uint32 | Type::Fixed32 => {
            if let Some(i) = f.to_u32() {
                ...
                Err(anyhow!(
                    "Can not construct an unsigned integer value from the given value, field {} type is {:?} but value is {:?}",
                    field_name,
                    descriptor.r#type(),
                    value
                ))
        ...
        _ => Err(anyhow!(
            "Only numeric field values can be configured with a numeric value, field {} type is {:?} but value is {:?}",
            field_name,
            descriptor.r#type(),
            value
        )),
        ...
        if !mrd.rules.is_empty() {
            for rule in &mrd.rules {
                match rule {
                    ...
                    Either::Right(mr) => {
                        return Err(anyhow!(
                            "Was expecting a value for '{}', but got a matching reference {:?}",
                            path,
                            mr
                        ));
                    }
...
fn value_for_field(mrd: &MatchingRuleDefinition) -> String {
    if mrd.value.is_empty() {
        if let Some(value_matcher) = mrd.rules.iter().find_map(|m| match m {
    ...
    match t {
        ...
        Type::Message => {
            if type_name == ".google.protobuf.BytesValue" {
                ...
                Err(anyhow!(
                    "value_for_type: Protobuf field {} has an unsupported type {:?} {}",
                    field_name,
                    t,
                    type_name
                ))
        ...
        Type::Enum => {
            let result = find_enum_value_by_name_in_message(
            ...
            if let Some((n, desc)) = result {
                ...
                Err(anyhow!(
                    "Protobuf enum value {} has no value {}",
                    type_name,
                    field_value
                ))
            }
        ...
        _ => Err(anyhow!(
            "Protobuf field {} has an unsupported type {:?}",
            field_name,
            t
        )),
    }
...
pub(crate) mod tests {
    ...
    use crate::message_builder::{MessageBuilder, MessageFieldValue, MessageFieldValueType, RType};
    use crate::protobuf::{
        build_embedded_message_field_value, build_field_value, build_single_embedded_field_value,
        configure_protobuf_service, construct_message_field,
        construct_protobuf_interaction_for_message, construct_protobuf_interaction_for_service,
        request_part, response_part, value_for_type,
    };
    ...
    fn construct_protobuf_interaction_for_message_test() {
        ...
        expect!(body.content_type.as_str()).to(be_equal_to(
            "application/protobuf;message=.test_package.test_message",
        ));
        ...
        expect!(body.content.as_ref()).to(be_some().value(&vec![
            ...
            5,  // 5 bytes
            48, 46, 48, 46, 48, 32, // field 4 varint encoded (4 << 3 + 0 == 32)
            210, 9, // 9 << 7 + 210 == 1234
        ]));
    ...
    const DESCRIPTORS_FOR_EACH_VALUE_TEST: [u8; 267] = [
        10, 136, 2, 10, 12, 115, 105, 109, 112, 108, 101, 46, 112, 114, 111, 116, 111, 34, 27, 10,
        9, 77, 101, 115, 115, 97, 103, 101, 73, 110, 18, 14, 10, 2, 105, 110, 24, 1, 32, 1, 40, 8,
        82, 2, 105, 110, 34, 30, 10, 10, 77, 101, 115, 115, 97, 103, 101, 79, 117, 116, 18, 16, 10,
        3, 111, 117, 116, 24, 1, 32, 1, 40, 8, 82, 3, 111, 117, 116, 34, 39, 10, 15, 86, 97, 108,
        117, 101, 115, 77, 101, 115, 115, 97, 103, 101, 73, 110, 18, 20, 10, 5, 118, 97, 108, 117,
        101, 24, 1, 32, 3, 40, 9, 82, 5, 118, 97, 108, 117, 101, 34, 40, 10, 16, 86, 97, 108, 117,
        101, 115, 77, 101, 115, 115, 97, 103, 101, 79, 117, 116, 18, 20, 10, 5, 118, 97, 108, 117,
        101, 24, 1, 32, 3, 40, 9, 82, 5, 118, 97, 108, 117, 101, 50, 96, 10, 4, 84, 101, 115, 116,
        18, 36, 10, 7, 71, 101, 116, 84, 101, 115, 116, 18, 10, 46, 77, 101, 115, 115, 97, 103,
        101, 73, 110, 26, 11, 46, 77, 101, 115, 115, 97, 103, 101, 79, 117, 116, 34, 0, 18, 50, 10,
        9, 71, 101, 116, 86, 97, 108, 117, 101, 115, 18, 16, 46, 86, 97, 108, 117, 101, 115, 77,
        101, 115, 115, 97, 103, 101, 73, 110, 26, 17, 46, 86, 97, 108, 117, 101, 115, 77, 101, 115,
        115, 97, 103, 101, 79, 117, 116, 34, 0, 98, 6, 112, 114, 111, 116, 111, 51,
    ];
    ...
    fn construct_protobuf_interaction_for_message_with_each_value_matcher() {
        ...
        let (message_descriptor, _) =
            find_message_descriptor_for_type(".ValuesMessageIn", &fds).unwrap();
        ...
        let body = result.contents.as_ref().unwrap();
        expect!(body.content_type.as_str())
            .to(be_equal_to("application/protobuf;message=.ValuesMessageIn"));
        expect!(body.content.as_ref()).to(be_some().value(&vec![
            10, // field 1 length encoded (1 << 3 + 2 == 10)
            32, // 32 bytes
    ...
    fn construct_message_field_with_message_with_each_value_matcher() {
        ...
        let mut matching_rules = MatchingRuleCategory::empty("body");
        let mut generators = hashmap! {};
        let file_descriptors: HashMap<String, &FileDescriptorProto> = fds
            .file
            .iter()
            .map(|des| (des.name.clone().unwrap_or_default(), des))
            .collect();
        ...
        expect!(inner.values.clone()).to(be_equal_to(vec![MessageFieldValue {
    ...
    fn build_field_value_with_message_with_each_value_matcher() {
        ...
        expect!(field_value.values.as_ref()).to(be_equal_to(vec![MessageFieldValue {
            name: "value".to_string(),
            raw_value: Some("00000000000000000000000000000000".to_string()),
            rtype: RType::String("00000000000000000000000000000000".to_string()),
        }]));
    ...
    fn construct_protobuf_interaction_for_service_returns_error_on_invalid_request_type() {
        ...
        let result = construct_protobuf_interaction_for_service(
            ...
            &config,
            "call",
            &hashmap! { "file".to_string() => &file_descriptor },
        );
    ...
    fn construct_protobuf_interaction_for_service_stores_the_expectations_against_the_interaction()
    {
        ...
        let (result, _) = configure_protobuf_service(
            "test_service/call",
            &config,
            &file_descriptor,
            &hashmap! { "test_file.proto".to_string() => &file_descriptor },
            "xxx",
        )
        .unwrap();
        let interaction_config = result
        ...
        expect!(interaction_config.fields.get("expectations").unwrap()).to(be_equal_to(
            &prost_types::Value {
                kind: Some(StructValue(Struct { fields: config })),
            },
        ));
    }
    ...
    fn build_embedded_message_field_value_with_repeated_field_configured_from_map_with_eachvalue_test()
     {
        ...
        let mut message_builder =
            MessageBuilder::new(&message_descriptor, "AreaResponse", &FILE_DESCRIPTOR);
        let path = DocPath::new("$.value").unwrap();
        ...
        let result = build_embedded_message_field_value(
            &mut message_builder,
            &path,
            &field_descriptor,
            "value",
    ...
    fn build_embedded_message_field_value_with_field_from_different_proto_file() {
        ...
        let main_descriptor = fds
            .file
            .iter()
            .find(|fd| fd.name.clone().unwrap_or_default() == "area_calculator.proto")
            .unwrap();
        let message_descriptor = main_descriptor
            .message_type
            .iter()
            .find(|md| md.name.clone().unwrap_or_default() == "ShapeMessage")
            .unwrap();
        let mut message_builder =
            MessageBuilder::new(&message_descriptor, "ShapeMessage", main_descriptor);
    ...
    pub(crate) const DESCRIPTOR_WITH_ENUM_BYTES: [u8; 1128] = [
        ...
        95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 46, 83, 113, 117, 97, 114, 101, 72, 0,
        82, 6, 115, 113, 117, 97, 114, 101, 18, 58, 10, 9, 114, 101, 99, 116, 97, 110, 103, 108,
        101, 24, 2, 32, 1, 40, 11, 50, 26, 46, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97,
        116, 111, 114, 46, 82, 101, 99, 116, 97, 110, 103, 108, 101, 72, 0, 82, 9, 114, 101, 99,
        116, 97, 110, 103, 108, 101, 18, 49, 10, 6, 99, 105, 114, 99, 108, 101, 24, 3, 32, 1, 40,
        11, 50, 23, 46, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 46, 67,
        105, 114, 99, 108, 101, 72, 0, 82, 6, 99, 105, 114, 99, 108, 101, 18, 55, 10, 8, 116, 114,
        105, 97, 110, 103, 108, 101, 24, 4, 32, 1, 40, 11, 50, 25, 46, 97, 114, 101, 97, 95, 99,
        97, 108, 99, 117, 108, 97, 116, 111, 114, 46, 84, 114, 105, 97, 110, 103, 108, 101, 72, 0,
        82, 8, 116, 114, 105, 97, 110, 103, 108, 101, 18, 70, 10, 13, 112, 97, 114, 97, 108, 108,
        101, 108, 111, 103, 114, 97, 109, 24, 5, 32, 1, 40, 11, 50, 30, 46, 97, 114, 101, 97, 95,
        99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 46, 80, 97, 114, 97, 108, 108, 101, 108, 111,
        103, 114, 97, 109, 72, 0, 82, 13, 112, 97, 114, 97, 108, 108, 101, 108, 111, 103, 114, 97,
        109, 66, 7, 10, 5, 115, 104, 97, 112, 101, 34, 41, 10, 6, 83, 113, 117, 97, 114, 101, 18,
        31, 10, 11, 101, 100, 103, 101, 95, 108, 101, 110, 103, 116, 104, 24, 1, 32, 1, 40, 2, 82,
        10, 101, 100, 103, 101, 76, 101, 110, 103, 116, 104, 34, 125, 10, 9, 82, 101, 99, 116, 97,
        110, 103, 108, 101, 18, 22, 10, 6, 108, 101, 110, 103, 116, 104, 24, 1, 32, 1, 40, 2, 82,
        6, 108, 101, 110, 103, 116, 104, 18, 20, 10, 5, 119, 105, 100, 116, 104, 24, 2, 32, 1, 40,
        2, 82, 5, 119, 105, 100, 116, 104, 18, 66, 10, 13, 97, 100, 95, 98, 114, 101, 97, 107, 95,
        116, 121, 112, 101, 24, 5, 32, 1, 40, 14, 50, 30, 46, 97, 114, 101, 97, 95, 99, 97, 108,
        99, 117, 108, 97, 116, 111, 114, 46, 65, 100, 66, 114, 101, 97, 107, 65, 100, 84, 121, 112,
        101, 82, 11, 97, 100, 66, 114, 101, 97, 107, 84, 121, 112, 101, 34, 32, 10, 6, 67, 105,
        114, 99, 108, 101, 18, 22, 10, 6, 114, 97, 100, 105, 117, 115, 24, 1, 32, 1, 40, 2, 82, 6,
        114, 97, 100, 105, 117, 115, 34, 79, 10, 8, 84, 114, 105, 97, 110, 103, 108, 101, 18, 21,
        10, 6, 101, 100, 103, 101, 95, 97, 24, 1, 32, 1, 40, 2, 82, 5, 101, 100, 103, 101, 65, 18,
        21, 10, 6, 101, 100, 103, 101, 95, 98, 24, 2, 32, 1, 40, 2, 82, 5, 101, 100, 103, 101, 66,
        18, 21, 10, 6, 101, 100, 103, 101, 95, 99, 24, 3, 32, 1, 40, 2, 82, 5, 101, 100, 103, 101,
        67, 34, 72, 10, 13, 80, 97, 114, 97, 108, 108, 101, 108, 111, 103, 114, 97, 109, 18, 31,
        10, 11, 98, 97, 115, 101, 95, 108, 101, 110, 103, 116, 104, 24, 1, 32, 1, 40, 2, 82, 10,
        98, 97, 115, 101, 76, 101, 110, 103, 116, 104, 18, 22, 10, 6, 104, 101, 105, 103, 104, 116,
        24, 2, 32, 1, 40, 2, 82, 6, 104, 101, 105, 103, 104, 116, 34, 68, 10, 11, 65, 114, 101, 97,
        82, 101, 113, 117, 101, 115, 116, 18, 53, 10, 6, 115, 104, 97, 112, 101, 115, 24, 1, 32, 3,
        40, 11, 50, 29, 46, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 46,
        83, 104, 97, 112, 101, 77, 101, 115, 115, 97, 103, 101, 82, 6, 115, 104, 97, 112, 101, 115,
        34, 36, 10, 12, 65, 114, 101, 97, 82, 101, 115, 112, 111, 110, 115, 101, 18, 20, 10, 5,
        118, 97, 108, 117, 101, 24, 1, 32, 3, 40, 2, 82, 5, 118, 97, 108, 117, 101, 42, 85, 10, 13,
        65, 100, 66, 114, 101, 97, 107, 65, 100, 84, 121, 112, 101, 18, 28, 10, 24, 77, 73, 83, 83,
        73, 78, 71, 95, 65, 68, 95, 66, 82, 69, 65, 75, 95, 65, 68, 95, 84, 89, 80, 69, 16, 0, 18,
        18, 10, 14, 65, 85, 68, 73, 79, 95, 65, 68, 95, 66, 82, 69, 65, 75, 16, 1, 18, 18, 10, 14,
        86, 73, 68, 69, 79, 95, 65, 68, 95, 66, 82, 69, 65, 75, 16, 2, 50, 173, 1, 10, 10, 67, 97,
        108, 99, 117, 108, 97, 116, 111, 114, 18, 78, 10, 12, 99, 97, 108, 99, 117, 108, 97, 116,
        101, 79, 110, 101, 18, 29, 46, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116,
        111, 114, 46, 83, 104, 97, 112, 101, 77, 101, 115, 115, 97, 103, 101, 26, 29, 46, 97, 114,
        101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 46, 65, 114, 101, 97, 82, 101,
        115, 112, 111, 110, 115, 101, 34, 0, 18, 79, 10, 14, 99, 97, 108, 99, 117, 108, 97, 116,
        101, 77, 117, 108, 116, 105, 18, 28, 46, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108,
        97, 116, 111, 114, 46, 65, 114, 101, 97, 82, 101, 113, 117, 101, 115, 116, 26, 29, 46, 97,
        114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 46, 65, 114, 101, 97, 82,
        101, 115, 112, 111, 110, 115, 101, 34, 0, 66, 28, 90, 23, 105, 111, 46, 112, 97, 99, 116,
        47, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 208, 2, 1, 98, 6,
    ...
    fn construct_message_field_with_global_enum_test() {
        ...
        let message_descriptor = main_descriptor
            .message_type
            .iter()
            .find(|md| md.name.clone().unwrap_or_default() == "Rectangle")
            .unwrap();
        let mut message_builder =
            MessageBuilder::new(&message_descriptor, "Rectangle", main_descriptor);
        ...
        let result = construct_message_field(
            ...
            &mut generators,
            "ad_break_type",
            &Value::String("AUDIO_AD_BREAK".to_string()),
            &path,
            &file_descriptors,
        );
        expect!(result).to(be_ok());
    ...
    pub(crate) const DESCRIPTOR_WITH_EMBEDDED_MESSAGE: [u8; 644] = [
        ...
        112, 114, 111, 116, 111, 18, 15, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116,
        111, 114, 34, 211, 2, 10, 14, 65, 100, 66, 114, 101, 97, 107, 82, 101, 113, 117, 101, 115,
        116, 18, 88, 10, 16, 97, 100, 95, 98, 114, 101, 97, 107, 95, 99, 111, 110, 116, 101, 120,
        116, 24, 1, 32, 3, 40, 11, 50, 46, 46, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97,
        116, 111, 114, 46, 65, 100, 66, 114, 101, 97, 107, 82, 101, 113, 117, 101, 115, 116, 46,
        65, 100, 66, 114, 101, 97, 107, 67, 111, 110, 116, 101, 120, 116, 82, 14, 97, 100, 66, 114,
        101, 97, 107, 67, 111, 110, 116, 101, 120, 116, 26, 230, 1, 10, 14, 65, 100, 66, 114, 101,
        97, 107, 67, 111, 110, 116, 101, 120, 116, 18, 36, 10, 14, 102, 111, 114, 99, 101, 100, 95,
        108, 105, 110, 101, 95, 105, 100, 24, 1, 32, 1, 40, 9, 82, 12, 102, 111, 114, 99, 101, 100,
        76, 105, 110, 101, 73, 100, 18, 44, 10, 18, 102, 111, 114, 99, 101, 100, 95, 99, 114, 101,
        97, 116, 105, 118, 101, 95, 105, 100, 24, 2, 32, 1, 40, 9, 82, 16, 102, 111, 114, 99, 101,
        100, 67, 114, 101, 97, 116, 105, 118, 101, 73, 100, 18, 30, 10, 11, 97, 100, 95, 98, 114,
        101, 97, 107, 95, 105, 100, 24, 3, 32, 1, 40, 9, 82, 9, 97, 100, 66, 114, 101, 97, 107, 73,
        100, 18, 28, 10, 9, 115, 101, 115, 115, 105, 111, 110, 73, 100, 24, 4, 32, 1, 40, 9, 82, 9,
        115, 101, 115, 115, 105, 111, 110, 73, 100, 18, 66, 10, 13, 97, 100, 95, 98, 114, 101, 97,
        107, 95, 116, 121, 112, 101, 24, 5, 32, 1, 40, 14, 50, 30, 46, 97, 114, 101, 97, 95, 99,
        97, 108, 99, 117, 108, 97, 116, 111, 114, 46, 65, 100, 66, 114, 101, 97, 107, 65, 100, 84,
        121, 112, 101, 82, 11, 97, 100, 66, 114, 101, 97, 107, 84, 121, 112, 101, 34, 36, 10, 12,
        65, 114, 101, 97, 82, 101, 115, 112, 111, 110, 115, 101, 18, 20, 10, 5, 118, 97, 108, 117,
        101, 24, 1, 32, 3, 40, 2, 82, 5, 118, 97, 108, 117, 101, 42, 85, 10, 13, 65, 100, 66, 114,
        101, 97, 107, 65, 100, 84, 121, 112, 101, 18, 28, 10, 24, 77, 73, 83, 83, 73, 78, 71, 95,
        65, 68, 95, 66, 82, 69, 65, 75, 95, 65, 68, 95, 84, 89, 80, 69, 16, 0, 18, 18, 10, 14, 65,
        85, 68, 73, 79, 95, 65, 68, 95, 66, 82, 69, 65, 75, 16, 1, 18, 18, 10, 14, 86, 73, 68, 69,
        79, 95, 65, 68, 95, 66, 82, 69, 65, 75, 16, 2, 50, 94, 10, 10, 67, 97, 108, 99, 117, 108,
        97, 116, 111, 114, 18, 80, 10, 12, 99, 97, 108, 99, 117, 108, 97, 116, 101, 79, 110, 101,
        18, 31, 46, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 46, 65,
        100, 66, 114, 101, 97, 107, 82, 101, 113, 117, 101, 115, 116, 26, 29, 46, 97, 114, 101, 97,
        95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 46, 65, 114, 101, 97, 82, 101, 115, 112,
        111, 110, 115, 101, 34, 0, 66, 28, 90, 23, 105, 111, 46, 112, 97, 99, 116, 47, 97, 114,
        101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 208, 2, 1, 98, 6, 112, 114, 111,
        116, 111, 51,
    ];
    ...
    fn build_single_embedded_field_value_with_embedded_message() {
        ...
        let message_descriptor = main_descriptor
            .message_type
            .iter()
            .find(|md| md.name.clone().unwrap_or_default() == "AdBreakRequest")
            .unwrap();
        let mut message_builder =
            MessageBuilder::new(&message_descriptor, "AdBreakRequest", main_descriptor);
    ...
    const DESCRIPTORS_ROUTE_GUIDE_WITH_ENUM_BASIC: [u8; 320] = [
        10, 189, 2, 10, 15, 116, 101, 115, 116, 95, 101, 110, 117, 109, 46, 112, 114, 111, 116,
        111, 18, 13, 114, 111, 117, 116, 101, 103, 117, 105, 100, 101, 46, 118, 50, 34, 65, 10, 5,
        80, 111, 105, 110, 116, 18, 26, 10, 8, 108, 97, 116, 105, 116, 117, 100, 101, 24, 1, 32, 1,
        40, 5, 82, 8, 108, 97, 116, 105, 116, 117, 100, 101, 18, 28, 10, 9, 108, 111, 110, 103,
        105, 116, 117, 100, 101, 24, 2, 32, 1, 40, 5, 82, 9, 108, 111, 110, 103, 105, 116, 117,
        100, 101, 34, 58, 10, 7, 70, 101, 97, 116, 117, 114, 101, 18, 47, 10, 6, 114, 101, 115,
        117, 108, 116, 24, 1, 32, 1, 40, 14, 50, 23, 46, 114, 111, 117, 116, 101, 103, 117, 105,
        100, 101, 46, 118, 50, 46, 84, 101, 115, 116, 69, 110, 117, 109, 82, 6, 114, 101, 115, 117,
        108, 116, 42, 56, 10, 8, 84, 101, 115, 116, 69, 110, 117, 109, 18, 14, 10, 10, 86, 65, 76,
        85, 69, 95, 90, 69, 82, 79, 16, 0, 18, 13, 10, 9, 86, 65, 76, 85, 69, 95, 79, 78, 69, 16,
        1, 18, 13, 10, 9, 86, 65, 76, 85, 69, 95, 84, 87, 79, 16, 2, 50, 69, 10, 4, 84, 101, 115,
        116, 18, 61, 10, 11, 71, 101, 116, 70, 101, 97, 116, 117, 114, 101, 50, 18, 20, 46, 114,
        111, 117, 116, 101, 103, 117, 105, 100, 101, 46, 118, 50, 46, 80, 111, 105, 110, 116, 26,
        22, 46, 114, 111, 117, 116, 101, 103, 117, 105, 100, 101, 46, 118, 50, 46, 70, 101, 97,
        116, 117, 114, 101, 34, 0, 66, 19, 90, 17, 105, 111, 46, 112, 97, 99, 116, 47, 116, 101,
        115, 116, 95, 101, 110, 117, 109, 98, 6, 112, 114, 111, 116, 111, 51,
    ];
    ...
    fn build_field_value_with_global_enum() {
        ...
        let message_descriptor = main_descriptor
            .message_type
            .iter()
            .find(|md| md.name.clone().unwrap_or_default() == "Feature")
            .unwrap();
        let mut message_builder =
            MessageBuilder::new(&message_descriptor, "Feature", main_descriptor);
    ...
    const DESCRIPTORS_MAP_WITH_PRIMITIVE_FIELDS: [u8; 334] = [
        10, 203, 2, 10, 10, 109, 97, 112, 115, 46, 112, 114, 111, 116, 111, 18, 4, 109, 97, 112,
        115, 34, 169, 1, 10, 13, 65, 99, 116, 105, 111, 110, 82, 101, 113, 117, 101, 115, 116, 18,
        22, 10, 6, 97, 99, 116, 105, 111, 110, 24, 1, 32, 1, 40, 9, 82, 6, 97, 99, 116, 105, 111,
        110, 18, 52, 10, 5, 112, 97, 114, 97, 109, 24, 2, 32, 3, 40, 11, 50, 30, 46, 109, 97, 112,
        115, 46, 65, 99, 116, 105, 111, 110, 82, 101, 113, 117, 101, 115, 116, 46, 80, 97, 114, 97,
        109, 69, 110, 116, 114, 121, 82, 5, 112, 97, 114, 97, 109, 18, 16, 10, 3, 105, 100, 115,
        24, 3, 32, 3, 40, 9, 82, 3, 105, 100, 115, 26, 56, 10, 10, 80, 97, 114, 97, 109, 69, 110,
        116, 114, 121, 18, 16, 10, 3, 107, 101, 121, 24, 1, 32, 1, 40, 9, 82, 3, 107, 101, 121, 18,
        20, 10, 5, 118, 97, 108, 117, 101, 24, 2, 32, 1, 40, 9, 82, 5, 118, 97, 108, 117, 101, 58,
        2, 56, 1, 34, 56, 10, 14, 65, 99, 116, 105, 111, 110, 82, 101, 115, 112, 111, 110, 115,
        101, 18, 38, 10, 14, 114, 101, 115, 112, 111, 110, 115, 101, 83, 116, 97, 116, 117, 115,
        24, 1, 32, 1, 40, 8, 82, 14, 114, 101, 115, 112, 111, 110, 115, 101, 83, 116, 97, 116, 117,
        115, 50, 73, 10, 6, 79, 77, 67, 97, 108, 99, 18, 63, 10, 18, 104, 97, 110, 100, 108, 101,
        66, 97, 116, 99, 104, 82, 101, 113, 117, 101, 115, 116, 18, 19, 46, 109, 97, 112, 115, 46,
        65, 99, 116, 105, 111, 110, 82, 101, 113, 117, 101, 115, 116, 26, 20, 46, 109, 97, 112,
        115, 46, 65, 99, 116, 105, 111, 110, 82, 101, 115, 112, 111, 110, 115, 101, 98, 6, 112,
        114, 111, 116, 111, 51,
    ];
    ...
    fn configure_message_with_map_with_primitive_fields() {
        ...
        let message_descriptor = main_descriptor
            .message_type
            .iter()
            .find(|md| md.name.clone().unwrap_or_default() == "ActionRequest")
            .unwrap();
        let mut message_builder =
            MessageBuilder::new(&message_descriptor, "ActionRequest", main_descriptor);
        ...
        expect!(&constructed.values).to(be_equal_to(&vec![
            MessageFieldValue {
                name: "key".to_string(),
                raw_value: Some("apply".to_string()),
                rtype: RType::String("apply".to_string()),
            },
            MessageFieldValue {
                name: "value".to_string(),
                raw_value: Some("Skip_holiday".to_string()),
                rtype: RType::String("Skip_holiday".to_string()),
            },
        ]));
    }
