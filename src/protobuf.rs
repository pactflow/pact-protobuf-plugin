...
use pact_models::matchingrules::expressions::{
    is_matcher_def, parse_matcher_def, MatchingRuleDefinition, ValueType,
};
...
use crate::metadata::{process_metadata, MessageMetadata};
use crate::protoc::Protoc;
use crate::utils::{
    find_enum_value_by_name, find_enum_value_by_name_in_message, find_message_descriptor,
    find_nested_type, is_map_field, is_repeated_field, prost_string, split_name,
};
...
fn configure_protobuf_service(
    ...
    debug!(">> configure_protobuf_service({service_name}, {config:?}, {descriptor_hash})");

    debug!(
        "Looking for service and method with name '{}'",
        service_name
    );
    let (service, proc_name) = service_name.split_once('/').ok_or_else(|| {
        anyhow!(
            "Service name '{}' is not valid, it should be of the form <SERVICE>/<METHOD>",
            service_name
        )
    })?;
    let service_descriptor = descriptor
        .service
        .iter()
        .find(|p| p.name.clone().unwrap_or_default() == service)
        .ok_or_else(|| anyhow!("Did not find a descriptor for service '{}'", service_name))?;
    debug!("service_descriptor = {:?}", service_descriptor);
...
fn construct_protobuf_interaction_for_service(
    ...
    debug!(method_name, service_part, "looking up method descriptor");
    let method_descriptor = descriptor
        .method
        .iter()
    ...
    info!(%input_name, ?input_package, input_message_name, "Input message");
    info!(%output_name, ?output_package, output_message_name, "Output message");

    let request_descriptor = find_message_descriptor(
        input_message_name,
        input_package,
        file_descriptor,
        all_descriptors,
    )?;
    let response_descriptor = find_message_descriptor(
        output_message_name,
        output_package,
        file_descriptor,
        all_descriptors,
    ...
    info!("request_descriptor = {:?}", request_descriptor);
    info!("response_descriptor = {:?}", response_descriptor);

    let request_part_config: BTreeMap<String, prost_types::Value> =
        request_part(config, service_part)?;
    info!(config = ?request_part_config, service_part, "Processing request part config");
    ...
    info!(config = ?response_part_config, service_part, "Processing response part config");
...
fn construct_protobuf_interaction_for_message(
    ...
    debug!("message_descriptor = {:?}", message_descriptor);
...
fn construct_message_field(
    ...
    if !field_name.starts_with("pact:") {
        if let Some(field) = message_builder.field_by_name(field_name) {
            debug!(?field_name, descriptor = ?field, "Found a descriptor for field");
            match field.r#type {
                Some(r#type) => {
                    if r#type == Type::Message as i32 {
                        // Embedded message
                        debug!(?field_name, "Field is for an embedded message");
                        ...
                        debug!(?field_name, "Field is not an embedded message");
...
fn build_struct_field(
    ...
    match field_value {
        Value::Object(map) => {
            if let Some(matching_def) = map.get("pact:match") {
    ...
    match value {
        ...
        Value::Array(a) => {
            ...
            let list = prost_types::ListValue {
                values: values.map(|v| v.unwrap()).collect(),
            };
            Ok(prost_types::Value {
...
pub(crate) mod tests {
    ...
    use prost_types::{
        ...
        FileDescriptorSet, MethodDescriptorProto, MethodOptions, OneofDescriptorProto,
        ServiceDescriptorProto, Struct,
    };
    use serde_json::{json, Value};
    ...
    use crate::protobuf::{
        ...
        construct_message_field, construct_protobuf_interaction_for_message,
        construct_protobuf_interaction_for_service, request_part, response_part, value_for_type,
    };
    ...
    fn construct_protobuf_interaction_for_message_test() {
        ...
        expect!(body.content_type.as_str())
            .to(be_equal_to("application/protobuf;message=test_message"));
    ...
    pub const DESCRIPTOR_BYTES: &str =
        "CrYHCgxjb21tb24ucHJvdG8SD2FyZWFfY2FsY3VsYXRvciL9AwoPTGlzdGVuZXJDb\
    ...
    W1wbGVzL2dycGMvcHJpbWFyeWIGcHJvdG8z";
        ...
        let result =
            super::find_message_descriptor("RectangleLocationRequest", None, file_descriptor, &all)
                .unwrap();
        expect!(result.field.len()).to(be_equal_to(2));
        let result = super::find_message_descriptor(
            "RectangleLocationRequest",
            Some("primary"),
            file_descriptor,
            &all,
        )
