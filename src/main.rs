        ...
        if let Some(auth) = request.metadata().get("authorization") {
            if let Ok(auth) = auth.to_str() {
                ...
                Err(Status::unauthenticated(
                    "could not read credentials supplied",
                ))
            }
        } else {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let formatting_layer =
        BunyanFormattingLayer::new("pact-protobuf-plugin".into(), json_non_blocking);
    let subscriber = FmtSubscriber::builder()
        .with_max_level(
            tracing_core::LevelFilter::from_str(log_level.as_str())
            ...
            interval.tick().await;
            {
                ...
                if let Some(i) = &*guard {
                    if i.elapsed().as_secs() > timeout {
                        info!(
                            "No activity for more than {timeout} seconds, sending shutdown signal"
                        );
                        elapsed = true;
                    }
                }
    ...
    Server::builder()
        .layer(layer)
        .add_service(PactPluginServer::with_interceptor(
            plugin,
            AuthInterceptor { server_key },
        ))
