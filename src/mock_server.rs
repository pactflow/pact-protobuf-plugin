...
use crate::utils::{
    build_grpc_route, find_message_descriptor_for_type, lookup_service_descriptors_for_interaction,
    parse_grpc_route, to_fully_qualified_name,
};
...
impl GrpcMockServer {
    ...
    pub async fn start_server(
        ...
        self.routes = self
            ...
            .filter_map(
                |i| match lookup_service_descriptors_for_interaction(&i, &self.pact) {
                    Ok((file_set, service, method, file)) => {
                        Some((file_set, service, method, file, i.clone()))
                    }
                    Err(_) => None,
                },
            )
                ...
                match to_fully_qualified_name(service.name(), file.package()) {
                    Ok(service_full_name) => {
                        match build_grpc_route(service_full_name.as_str(), method.name()) {
                            Ok(route) => Some((
                                route,
                                (file_set.clone(), file.clone(), method.clone(), i.clone()),
                            )),
                            Err(_) => None,
                        }
                    }
