...
use maplit::btreemap;
use prost::Message;
use prost::encoding::{WireType, encode_key, encode_varint, string};
...
use crate::utils::{display_bytes, last_name, should_be_packed_type};
...
impl MessageBuilder {
    ...
    pub fn add_repeated_field_value(
        &mut self,
        field_descriptor: &FieldDescriptorProto,
        field_name: &str,
        field_value: MessageFieldValue,
    ...
    pub fn add_map_field_value(
        &mut self,
        field_descriptor: &FieldDescriptorProto,
        field_name: &str,
        key: MessageFieldValue,
    ...
    pub fn encode_message(&self) -> anyhow::Result<Bytes> {
        ...
        for (_, field_data) in self.fields.iter().sorted_by(|(_, a), (_, b)| {
            Ord::cmp(
                &a.descriptor.number.unwrap_or_default(),
                &b.descriptor.number.unwrap_or_default(),
            )
        }) {
            match field_data.field_type {
                MessageFieldValueType::Normal => self.encode_single_field(
                    &mut buffer,
                    field_data,
                    field_data.values.first().cloned(),
                )?,
                MessageFieldValueType::Map => self.encode_map_field(&mut buffer, field_data)?,
                MessageFieldValueType::Repeated => {
                    self.encode_repeated_field(&mut buffer, field_data)?
                }
            }
        }
    ...
    fn encode_single_field(
        ...
        field_data: &FieldValueInner,
        value: Option<MessageFieldValue>,
    ) -> anyhow::Result<()> {
        trace!(
            ">> encode_single_field({:?}, {:?}, {:?})",
            value, field_data.descriptor.number, field_data
        );
        if let Some(value) = value {
            if let Some(tag) = field_data.descriptor.number {
                match field_data.proto_type {
                    Type::Double => prost::encoding::double::encode(
                        tag as u32,
                        &value.rtype.as_f64()?,
                        &mut buffer,
                    ),
                    Type::Float => prost::encoding::float::encode(
                        tag as u32,
                        &value.rtype.as_f32()?,
                        &mut buffer,
                    ),
                    Type::Int64 => prost::encoding::int64::encode(
                        tag as u32,
                        &value.rtype.as_i64()?,
                        &mut buffer,
                    ),
                    Type::Uint64 => prost::encoding::uint64::encode(
                        tag as u32,
                        &value.rtype.as_u64()?,
                        &mut buffer,
                    ),
                    Type::Int32 => prost::encoding::int32::encode(
                        tag as u32,
                        &value.rtype.as_i32()?,
                        &mut buffer,
                    ),
                    Type::Uint32 => prost::encoding::uint32::encode(
                        tag as u32,
                        &value.rtype.as_u32()?,
                        &mut buffer,
                    ),
                    Type::Fixed64 => prost::encoding::fixed64::encode(
                        tag as u32,
                        &value.rtype.as_u64()?,
                        &mut buffer,
                    ),
                    Type::Fixed32 => prost::encoding::fixed32::encode(
                        tag as u32,
                        &value.rtype.as_u32()?,
                        &mut buffer,
                    ),
                    Type::Bool => prost::encoding::bool::encode(
                        tag as u32,
                        &value.rtype.as_bool()?,
                        &mut buffer,
                    ),
                    ...
                    Type::Message => match &value.rtype {
                        ...
                        _ => {
                            return Err(anyhow!(
                                "Mismatched types, expected a message builder but got {:?}",
                                value.rtype
                            ));
                        }
                    },
                    Type::Bytes => {
                        if let RType::Bytes(b) = &value.rtype {
                            prost::encoding::bytes::encode(tag as u32, b, &mut buffer);
                        } else {
                            return Err(anyhow!(
                                "Mismatched types, expected a byte array but got {:?}",
                                value.rtype
                            ));
                        }
                    }
                    Type::Enum => {
                        if let RType::Enum(n, desc) = &value.rtype {
                            self.encode_enum_value(
                            ...
                            return Err(anyhow!(
                                "Mismatched types, expected an enum but got {:?}",
                                value.rtype
                            ));
                        }
                    }
                    Type::Sfixed32 => prost::encoding::sfixed32::encode(
                        tag as u32,
                        &value.rtype.as_i32()?,
                        &mut buffer,
                    ),
                    Type::Sfixed64 => prost::encoding::sfixed64::encode(
                        tag as u32,
                        &value.rtype.as_i64()?,
                        &mut buffer,
                    ),
                    Type::Sint32 => prost::encoding::sint32::encode(
                        tag as u32,
                        &value.rtype.as_i32()?,
                        &mut buffer,
                    ),
                    Type::Sint64 => prost::encoding::sint64::encode(
                        tag as u32,
                        &value.rtype.as_i64()?,
                        &mut buffer,
                    ),
                    _ => {
                        return Err(anyhow!(
                            "Protobuf type {:?} is not supported",
                            field_data.proto_type
                        ));
                    }
                }
            }
    ...
    fn encode_enum_value(
        ...
        trace!(
            ">> encode_enum_value({:?}, {}, {})",
            field_value, tag, enum_value
        );
        let enum_type_name = descriptor.type_name.as_ref().ok_or_else(|| {
            anyhow!(
                "Type name is missing from the descriptor for enum field {}",
                field_value.name
            )
        })?;
        let enum_value = enum_proto
            .value
            .iter()
            .find(|enum_val| enum_val.number == Some(*enum_value))
            .ok_or_else(|| {
                anyhow!(
                    "Did not find the enum value {} for the enum {} in the Protobuf descriptor",
                    enum_value,
                    enum_type_name
                )
            })?;
        if let Some(enum_value_number) = enum_value.number {
            ...
            warn!(
                "Enum value {:?} for enum {} does not have a numeric value set, will use the default",
    ...
    fn encode_map_field(
        ...
        if !field_value.values.is_empty() {
            if field_value.values.len() % 2 == 1 {
                return Err(anyhow!(
                    "Map fields need to have an even number of field values as key-value pairs, got {} field values",
                    field_value.values.len()
                ));
            }
            let entry_type_name =
                field_value.descriptor.type_name.as_ref().ok_or_else(|| {
                    anyhow!("Type name is missing from the descriptor for map field")
                })?;
        ...
        .ok_or_else(|| anyhow!("Did not find the field descriptor for the value for the map field {} in the Protobuf descriptor", entry_type_name))?;
            ...
            let entries = field_value
                .values
                .iter()
                .tuples::<(_, _)>()
                .map(|(k, v)| MessageFieldValue {
            ...
            .sorted_by(|(_, a), (_, b)| Ord::cmp(&a.descriptor.number, &b.descriptor.number))
        {
            if let Some(field_num) = inner.descriptor.number {
                match inner.field_type {
                    MessageFieldValueType::Normal => buffer.push_str(
                        format!(
                            "{}    {} {} = {};\n",
                            indent,
                    ...
                    MessageFieldValueType::Map => buffer.push_str(
                        format!(
                            "{}    map<{}> {} = {};\n",
                            indent,
                            field_type_name(inner)?,
                    ...
                    MessageFieldValueType::Repeated => buffer.push_str(
                        format!(
                            "{}    repeated {} {} = {};\n",
                            indent,
                            field_type_name(inner)?,
    ...
    fn encode_packed_field(
        ...
        if let Some(tag) = field_value.descriptor.number {
            match field_value.proto_type {
                ...
                _ => Err(anyhow!(
                    "Can not encode a {:?} field in packaged form",
                    field_value.proto_type
                )),
            }
        } else {
            Err(anyhow!(
                "Unable to encode field {:?} as it has no tag",
                field_value.descriptor.name
            ))
        }
    }
}
...
fn field_type_name(field: &FieldValueInner) -> anyhow::Result<String> {
    Ok(match field.proto_type {
        ...
        Type::Message => {
            let message = field.descriptor.type_name.as_ref().ok_or_else(|| {
                anyhow!("Type name is missing from the descriptor for message field")
            })?;
        ...
        Type::Enum => {
            let enum_type_name = field.descriptor.type_name.as_ref().ok_or_else(|| {
                anyhow!("Type name is missing from the descriptor for enum field")
            })?;
...
pub(crate) mod tests {
  ...
  zcG9uc2VCEFoOaW8ucGFjdC5wbHVnaW5iBnByb3RvMw==";
    ...
    fn encode_simple_message_test() {
        ...
        expect!(result.to_vec()).to(be_equal_to(
            BASE64
                .decode("ChJwbHVnaW4tZHJpdmVyLXJ1c3QSBTAuMC4w")
                .unwrap(),
        ));
    ...
    fn encode_message_bytes_test() {
        ...
        let field3 =
            enum_field_descriptor!("contentTypeHint", 3, ".io.pact.plugin.Body.ContentTypeHint");
        ...
        let descriptor = DescriptorProto {
            ...
            field: vec![field1.clone(), field2.clone(), field3.clone()],
            extension: vec![],
            nested_type: vec![],
        ...
        let mut bytes_message =
            MessageBuilder::new(&content_descriptor, "BytesValue", &file_descriptor);
        bytes_message.set_field_value(
            &bytes_field,
            "value",
            MessageFieldValue {
    ...
    fn get_file_descriptor(file_name: &str, descriptor: &str) -> Option<FileDescriptorProto> {
        ...
        fds.file
            .iter()
            .find(|fd| fd.name.clone().unwrap_or_default() == file_name)
            .cloned()
    }
    ...
    fn encode_message_with_map_field_test() {
        ...
        let mut message =
            MessageBuilder::new(&descriptor, "CompareContentsRequest", &file_descriptor);
        message.set_field_value(
            &field1,
            "allowUnexpectedKeys",
            MessageFieldValue {
        ...
        let rule_descriptor = DescriptorProto {
            ...
            field: vec![matching_rule_field.clone()],
            extension: vec![],
            nested_type: vec![],
            enum_type: vec![],
            extension_range: vec![],
        ...
        let mut matching_rules =
            MessageBuilder::new(&rule_descriptor, "MatchingRules", &file_descriptor);

        let type_field_descriptor = string_field_descriptor!("type", 1);
        let values_field_descriptor =
            message_field_descriptor!("values", 2, ".google.protobuf.Struct");
        ...
        let mut matching_rule_1 =
            MessageBuilder::new(&matching_rule_descriptor, "MatchingRule", &file_descriptor);
        matching_rule_1.set_field_value(
        ...
        matching_rules.add_repeated_field_value(
            &matching_rule_field,
            "rule",
            MessageFieldValue {
                name: "".to_string(),
        ...
        let mut matching_rule_2 =
            MessageBuilder::new(&matching_rule_descriptor, "MatchingRule", &file_descriptor);
        matching_rule_2.set_field_value(
            &type_field_descriptor,
            "type",
            MessageFieldValue {
                name: "type".to_string(),
        ...
        let struct_descriptor = DescriptorProto {
            ...
            field: vec![struct_fields_descriptor.clone()],
            extension: vec![],
            nested_type: vec![DescriptorProto {
        ...
        let value_descriptor = DescriptorProto {
            name: Some("Value".to_string()),
            field: vec![value_string_field.clone()],
            extension: vec![],
            nested_type: vec![],
            enum_type: vec![],
            extension_range: vec![],
        ...
        let mut matching_rule_values =
            MessageBuilder::new(&struct_descriptor, "Struct", &file_descriptor);
        matching_rule_values.add_map_field_value(
            &struct_fields_descriptor,
            "fields",
            MessageFieldValue {
                name: "key".to_string(),
                raw_value: None,
        ...
        rule2.add_repeated_field_value(
            &matching_rule_field,
            "rule",
            MessageFieldValue {
                name: "".to_string(),
        ...
        let encoded_fields =
            decode_message(&mut encoded_buf, &descriptor, &file_descriptor_set).unwrap();
        ...
        let actual_fields = result.iter().map(|f| f.field_num).sorted().collect_vec();
        let expected_fields = result.iter().map(|f| f.field_num).sorted().collect_vec();
        expect!(actual_fields).to(be_equal_to(expected_fields));

        expect!(result.iter().find(|f| f.field_num == 3)).to(be_equal_to(
            encoded_fields.iter().find(|f| f.field_num == 3),
        ));

        let expected_map_values = encoded_fields
            .iter()
            .filter(|f| f.field_num == 4)
            .collect_vec();
        let actual_map_values = result.iter().filter(|f| f.field_num == 4).collect_vec();
        expect!(actual_map_values.len()).to(be_equal_to(2));
        let actual_data = actual_map_values
            .iter()
            .map(|f| {
            ...
            .collect_vec();
        let expected_data = expected_map_values
            .iter()
            .map(|f| match &f.data {
                ProtobufFieldData::Message(d, _) => d.clone(),
                _ => panic!("Got an unexpected field type {:?}", f.data),
  ...
  FsY3VsYXRvci5BcmVhUmVzcG9uc2UiAEIcWhdpby5wYWN0L2FyZWFfY2FsY3VsYXRvctACAWIGcHJvdG8z";
    ...
    fn test_packed_repeated_fields() {
        let file_descriptor =
            get_file_descriptor("area_calculator.proto", AREA_CALCULATOR_DESCRIPTOR).unwrap();
        let area_response_descriptor = file_descriptor
            .message_type
            .iter()
            .find(|desc| desc.name.clone().unwrap_or_default() == "AreaResponse")
            .unwrap();
        let values_field_descriptor = area_response_descriptor
            .field
            .iter()
            .find(|desc| desc.name.clone().unwrap_or_default() == "value")
            .unwrap();
        let mut builder =
            MessageBuilder::new(area_response_descriptor, "AreaResponse", &file_descriptor);
  ...
  XNzdWUtZGVtby87cGFjdGlzc3VlYgZwcm90bzM=";
    ...
    fn repeated_enum_fields_must_be_packed() {
        let file_descriptor =
            get_file_descriptor("repeated_enum.proto", REPEATED_ENUM_DESCRIPTORS).unwrap();
        let request_descriptor = file_descriptor
            .message_type
            .iter()
            .find(|desc| desc.name.clone().unwrap_or_default() == "BrokenSampleRequest")
            .unwrap();
        let values_field_descriptor = request_descriptor
            .field
            .iter()
            .find(|desc| desc.name.clone().unwrap_or_default() == "type")
            .unwrap();
        let mut builder =
            MessageBuilder::new(request_descriptor, "BrokenSampleRequest", &file_descriptor);
        let enum_proto =
            find_enum_by_name_in_message(&request_descriptor.enum_type, "Type").unwrap();
        let message_field_value = MessageFieldValue {
            name: "type".to_string(),
    ...
    fn test_field_with_global_enum() {
        ...
        let value_field_descriptor = message_descriptor
            .field
            .iter()
