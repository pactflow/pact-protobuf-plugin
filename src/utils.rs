...
pub fn find_message_type_by_name(
    ...
    descriptors
        ...
        .iter()
        .map(|descriptor| {
            find_message_type_in_file_descriptor(message_name, descriptor)
                .map(|ds| (ds, descriptor))
                .ok()
        })
        ...
        .ok_or_else(|| {
            anyhow!(
                "Did not find a message type '{}' in the descriptors",
                message_name
            )
        })
}
...
pub fn field_data_to_json(
    ...
    for field in field_data {
        if let Some(value) = descriptor
            .field
            .iter()
            .find(|f| f.number.unwrap_or(-1) as u32 == field.field_num)
        {
            match &value.name {
                Some(name) => {
                    object.insert(
                        ...
                        match &field.data {
                            ...
                            ProtobufFieldData::Bytes(b) => {
                                serde_json::Value::Array(b.iter().map(|v| json!(v)).collect())
                            }
                            ProtobufFieldData::Enum(n, descriptor) => {
                                serde_json::Value::String(enum_name(*n, descriptor))
                            }
                            ProtobufFieldData::Message(b, descriptor) => {
                                let mut bytes = BytesMut::from(b.as_slice());
                                let message_data =
                                    decode_message(&mut bytes, descriptor, descriptors)?;
                ...
                None => warn!(
                    "Did not get the field name for field number {}",
                    field.field_num
                ),
            }
        } else {
            warn!(
                "Did not find the descriptor for field number {}",
                field.field_num
            );
        }
    }
...
pub(crate) fn lookup_service_descriptors_for_interaction(
    ...
    let (service_name, method_name) = service.split_once('/').ok_or_else(|| {
        anyhow!(
    ...
    let (file_descriptor, service_descriptor) =
        find_service_descriptor(&descriptors, service_name)?;
    let method_descriptor = service_descriptor
        .method
        .iter()
        .find(|method_desc| method_desc.name.clone().unwrap_or_default() == method_name)
        .ok_or_else(|| {
            anyhow!(
                "Did not find the method {} in the Protobuf file descriptor for service '{}'",
                method_name,
                service
            )
        })?;
...
pub fn get_descriptors_for_interaction(
    ...
    let descriptor_config = plugin_config
        .get(message_key)
        .ok_or_else(|| {
            anyhow!(
