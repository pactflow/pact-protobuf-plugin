...
use prost_types::{
    ...
    FileDescriptorSet, MethodDescriptorProto, ServiceDescriptorProto, Value,
    field_descriptor_proto,
};
use serde_json::{Map, json};
use tracing::{debug, error, instrument, trace, warn};

use crate::message_decoder::{ProtobufField, ProtobufFieldData, decode_message};
...
pub fn to_fully_qualified_name(name: &str, package: &str) -> anyhow::Result<String> {
    match name {
        "" => Err(anyhow!(
            "type name cannot be empty when constructing a fully qualified name"
        )),
        _ => Ok(match package {
            "" => format!(".{}", name),
...
pub fn find_message_type_in_file_descriptor(
    ...
    descriptor
        ...
        .ok_or_else(|| {
            anyhow!(
                "Did not find a message type '{}' in the file descriptor '{}'",
                message_name,
...
pub fn find_method_descriptor_for_service(
    ...
    let method_descriptor = service_descriptor
        .method
        .iter()
        .find(|method_desc| method_desc.name() == method_name)
        .cloned()
        .ok_or_else(|| {
            anyhow!(
                "Did not find the method {} in the Protobuf descriptor for service '{}'",
                method_name,
                service_descriptor.name()
            )
        })?;
    trace!(
        "Found method descriptor {:?} for method {}",
...
pub(crate) fn find_message_descriptor(
    ...
    if package.is_some() {
        trace!(
            "Looking for message descriptor for message '{}' in package '{:?}'",
            message_name, package
        );
    } else {
        trace!(
            "Looking for message descriptor for message '{}'",
            message_name
        );
    }
    ...
    descriptors
        .iter()
        .find_map(|fd| {
            find_message_type_in_file_descriptor(message_name, fd)
                .ok()
...
pub(crate) fn find_service_descriptor(
    ...
    if package.is_some() {
        debug!(
            "Looking for service '{}' with package '{:?}'",
...
fn find_all_file_descriptors_for_package(
    ...
    if found.is_empty() {
        Err(anyhow!(
            "Did not find any file descriptors for a package '{}'",
            package
        ))
    } else {
        debug!(
...
fn find_all_file_descriptors_with_no_package(
    ...
    let found: Vec<_> = all_descriptors
        .iter()
        .filter(|d| d.package.is_none())
        .cloned()
        .collect();
    if found.is_empty() {
        Err(anyhow!(
            "Did not find any file descriptors with no package specified"
        ))
    } else {
...
pub fn find_nested_type(
    ...
    if field.r#type() == Type::Message {
        ...
        trace!(
            "find_nested_type: Looking for nested type '{}'",
            message_type
        );
        message_descriptor
            .nested_type
...
pub fn find_message_field_by_name(
    descriptor: &DescriptorProto,
    field_data: Vec<ProtobufField>,
    field_name: &str,
) -> Option<ProtobufField> {
    let field_num = match descriptor
        .field
        .iter()
    ...
    field_data
        .iter()
        .find(|d| d.field_num == field_num as u32)
        .cloned()
}
...
pub fn enum_name(enum_value: i32, descriptor: &EnumDescriptorProto) -> String {
    descriptor
        .value
        .iter()
        .find(|v| v.number.unwrap_or(-1) == enum_value)
        .map(|v| {
            v.name
                .clone()
                .unwrap_or_else(|| format!("enum {}", enum_value))
        })
...
pub fn find_enum_value_by_name_in_message(
    ...
    enum_types.iter().find_map(|enum_descriptor| {
        trace!(
            "find_enum_value_by_name_in_message: enum type = {:?}",
...
pub fn find_enum_by_name_in_message(
    ...
    enum_types.iter().find_map(|enum_descriptor| {
        trace!(
            "find_enum_by_name_in_message: enum type = {:?}",
            enum_descriptor.name
        );
        if let Some(name) = &enum_descriptor.name {
            if name == last_name(enum_name) {
...
pub fn find_enum_by_name(
    ...
    let enum_name_full = enum_name
        ...
        .collect::<Vec<_>>()
        .join(".");
    let result = descriptors.file.iter().find_map(|fd| {
    ...
    for field in field_data {
        if let ProtobufFieldData::Message(b, entry_descriptor) = &field.data {
            ...
            if message_data.len() == 2 {
                ...
                return Err(anyhow!(
                    "Was expecting 2 values (key, value) for the entry with field number {}, but got {:?}",
                    field.field_num,
                    message_data
                ));
            }
            ...
            return Err(anyhow!(
                "Was expecting a message for the entry with field number {}, but got {}",
                field.field_num,
                field.data
            ));
        }
...
fn proto_value_to_json(
    ...
    match &value_field.data {
                ...
                match &field_data.data {
                    ...
                    ProtobufFieldData::Message(m, desc) => {
                        if desc.name() == "ListValue" {
                            ...
                            Err(anyhow!(
                                "{} is not a valid value for a Struct entry",
                                field_data.data.type_name()
                            ))
                        }
                    }
        ...
        _ => Err(anyhow!(
            "Found an unrecognisable type for a Google Struct field {}",
            value_field.data.type_name()
        )),
    }
}

/// Parse the JSON string into a V4 Pact model
pub(crate) fn parse_pact_from_request_json(
    pact_json: &str,
    source: &str,
) -> anyhow::Result<V4Pact> {
...
pub fn lookup_interaction_config(
    interaction: &dyn V4Interaction,
) -> Option<HashMap<String, serde_json::Value>> {
    interaction.plugin_config().iter().find_map(|(key, value)| {
...
pub(crate) fn lookup_service_descriptors_for_interaction(
    ...
    let descriptor_key = interaction_config
        .get("descriptorKey")
        .map(json_to_string)
    ...
    let (file_descriptor, service_descriptor) =
        find_service_descriptor_for_type(service_with_package, &descriptors)?;
...
fn get_descriptor_config<'a>(
    ...
    plugin_config
        ...
        .ok_or_else(|| {
            anyhow!(
                "Plugin configuration item with key '{}' is required. Received config {:?}",
                message_key,
                plugin_config.keys()
            )
        })?
        ...
        .ok_or_else(|| {
            anyhow!(
                "Plugin configuration item with key '{}' has an invalid format",
                message_key
            )
        })
}
...
pub fn get_descriptors_for_interaction(
    ...
    let descriptor_bytes_encoded = descriptor_config
        .get("protoDescriptors")
        .map(json_to_string)
        .unwrap_or_default();
    if descriptor_bytes_encoded.is_empty() {
        return Err(anyhow!(
            "Plugin configuration item with key '{}' is required, but the descriptors were empty. Received config {:?}",
            message_key,
            plugin_config.keys()
        ));
    }
    ...
    let descriptor_bytes = match BASE64.decode(descriptor_bytes_encoded) {
        ...
        Err(err) => {
            return Err(anyhow!(
                "Failed to decode the Protobuf descriptor - {}",
                err
            ));
        }
    };
    debug!(
        "Protobuf file descriptor set is {} bytes",
    ...
    if descriptor_hash != message_key {
        return Err(anyhow!(
            "Protobuf descriptors checksum failed. Expected {} but got {}",
            message_key,
            descriptor_hash
        ));
    }
    ...
    FileDescriptorSet::decode(descriptor_bytes).map_err(|err| anyhow!(err))
}
...
pub fn proto_value_to_map(val: &Value) -> anyhow::Result<BTreeMap<String, Value>> {
    match &val.kind {
        Some(kind) => match kind {
            ...
            _ => Err(anyhow!(
                "Must be a Protobuf Struct or NullValue, got {}",
                type_of(kind)
            )),
        },
...
pub(crate) mod tests {
    ...
    use super::{
        build_grpc_route, find_file_descriptors, find_message_descriptor_for_type,
        find_method_descriptor_for_service, find_service_descriptor_for_type, parse_grpc_route,
        split_service_and_method,
    };
    use crate::message_decoder::{ProtobufField, ProtobufFieldData};
    use crate::utils::{
        as_hex, find_enum_value_by_name, find_nested_type, is_map_field, last_name, parse_name,
        struct_field_data_to_json, to_fully_qualified_name,
    };
    use base64::Engine;
    use base64::engine::general_purpose::STANDARD as BASE64;
    ...
    fn split_service_and_method_test() {
        ...
        expect!(split_service_and_method("service/method").unwrap())
            .to(be_equal_to(("service", "method")));
        // TODO: we don't support this case either way - maybe we should error out if there's more than one slash?
        expect!(split_service_and_method("service/subservice/method").unwrap())
            .to(be_equal_to(("service", "subservice/method")));
    }

    #[test]
    fn to_fully_qualified_name_test() {
        expect!(to_fully_qualified_name("service", "package").unwrap())
            .to(be_equal_to(".package.service"));
        expect!(to_fully_qualified_name("service", "package.with.dots").unwrap())
            .to(be_equal_to(".package.with.dots.service"));
    ...
    fn test_build_grpc_route() {
        // Valid inputs
        expect!(build_grpc_route(".com.example.Service", "Method").unwrap())
            .to(be_equal_to("/com.example.Service/Method"));
        expect!(build_grpc_route("com.example.Service", "Method").unwrap())
            .to(be_equal_to("/com.example.Service/Method"));
    ...
    fn test_parse_grpc_route() {
        // Valid inputs
        expect!(parse_grpc_route("/com.example.Service/Method"))
            .to(be_some().value((".com.example.Service".to_string(), "Method".to_string())));
        expect!(parse_grpc_route("/com.example.Service/Method/"))
            .to(be_some().value((".com.example.Service".to_string(), "Method".to_string())));
    ...
    pub(crate) const DESCRIPTOR_WITH_EXT_MESSAGE: [u8; 626] = [
        10, 168, 2, 10, 11, 86, 97, 108, 117, 101, 46, 112, 114, 111, 116, 111, 18, 21, 97, 114,
        101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 46, 86, 97, 108, 117, 101, 34,
        162, 1, 10, 14, 65, 100, 66, 114, 101, 97, 107, 67, 111, 110, 116, 101, 120, 116, 18, 36,
        10, 14, 102, 111, 114, 99, 101, 100, 95, 108, 105, 110, 101, 95, 105, 100, 24, 1, 32, 1,
        40, 9, 82, 12, 102, 111, 114, 99, 101, 100, 76, 105, 110, 101, 73, 100, 18, 44, 10, 18,
        102, 111, 114, 99, 101, 100, 95, 99, 114, 101, 97, 116, 105, 118, 101, 95, 105, 100, 24, 2,
        32, 1, 40, 9, 82, 16, 102, 111, 114, 99, 101, 100, 67, 114, 101, 97, 116, 105, 118, 101,
        73, 100, 18, 30, 10, 11, 97, 100, 95, 98, 114, 101, 97, 107, 95, 105, 100, 24, 3, 32, 1,
        40, 9, 82, 9, 97, 100, 66, 114, 101, 97, 107, 73, 100, 18, 28, 10, 9, 115, 101, 115, 115,
        105, 111, 110, 73, 100, 24, 4, 32, 1, 40, 9, 82, 9, 115, 101, 115, 115, 105, 111, 110, 73,
        100, 42, 85, 10, 13, 65, 100, 66, 114, 101, 97, 107, 65, 100, 84, 121, 112, 101, 18, 28,
        10, 24, 77, 73, 83, 83, 73, 78, 71, 95, 65, 68, 95, 66, 82, 69, 65, 75, 95, 65, 68, 95, 84,
        89, 80, 69, 16, 0, 18, 18, 10, 14, 65, 85, 68, 73, 79, 95, 65, 68, 95, 66, 82, 69, 65, 75,
        16, 1, 18, 18, 10, 14, 86, 73, 68, 69, 79, 95, 65, 68, 95, 66, 82, 69, 65, 75, 16, 2, 98,
        6, 112, 114, 111, 116, 111, 51, 10, 196, 2, 10, 21, 97, 114, 101, 97, 95, 99, 97, 108, 99,
        117, 108, 97, 116, 111, 114, 46, 112, 114, 111, 116, 111, 18, 15, 97, 114, 101, 97, 95, 99,
        97, 108, 99, 117, 108, 97, 116, 111, 114, 26, 11, 86, 97, 108, 117, 101, 46, 112, 114, 111,
        116, 111, 34, 97, 10, 14, 65, 100, 66, 114, 101, 97, 107, 82, 101, 113, 117, 101, 115, 116,
        18, 79, 10, 16, 97, 100, 95, 98, 114, 101, 97, 107, 95, 99, 111, 110, 116, 101, 120, 116,
        24, 1, 32, 3, 40, 11, 50, 37, 46, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116,
        111, 114, 46, 86, 97, 108, 117, 101, 46, 65, 100, 66, 114, 101, 97, 107, 67, 111, 110, 116,
        101, 120, 116, 82, 14, 97, 100, 66, 114, 101, 97, 107, 67, 111, 110, 116, 101, 120, 116,
        ...
        67, 97, 108, 99, 117, 108, 97, 116, 111, 114, 18, 80, 10, 12, 99, 97, 108, 99, 117, 108,
        97, 116, 101, 79, 110, 101, 18, 31, 46, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108,
        97, 116, 111, 114, 46, 65, 100, 66, 114, 101, 97, 107, 82, 101, 113, 117, 101, 115, 116,
        26, 29, 46, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114, 46, 65,
        114, 101, 97, 82, 101, 115, 112, 111, 110, 115, 101, 34, 0, 66, 28, 90, 23, 105, 111, 46,
        112, 97, 99, 116, 47, 97, 114, 101, 97, 95, 99, 97, 108, 99, 117, 108, 97, 116, 111, 114,
        208, 2, 1, 98, 6, 112, 114, 111, 116, 111, 51,
    ];
    ...
    fn find_message_descriptor_for_type_ext_test() {
        ...
        let (result, file_descriptor) =
            find_message_descriptor_for_type(".area_calculator.Value.AdBreakContext", &fds)
                .unwrap();
    ...
    fn find_nested_type_test() {
        ...
        expect!(find_nested_type(&message, &field_with_matching_type_name))
            .to(be_some().value(nested));
    }
    ...
    fn is_map_field_test() {
        ...
        let message = DescriptorProto {
            ...
            nested_type: vec![non_map_nested, map_nested],
            ..DescriptorProto::default()
        };
    ...
    fn find_message_descriptor_for_type_test() {
        ...
        let request_file: FileDescriptorProto = FileDescriptorProto {
            ...
            message_type: vec![request_msg.clone(), another_request_msg.clone()],
            ..FileDescriptorProto::default()
        };
        let request_file2: FileDescriptorProto = FileDescriptorProto {
            name: Some("request.proto".to_string()),
            package: Some("service2".to_string()),
            message_type: vec![request_msg.clone()],
            ..FileDescriptorProto::default()
        };
        let all_descriptors = FileDescriptorSet {
            file: vec![request_file.clone(), request_file2.clone()],
        };
        // fully qualified name
        let (md, fd) =
            find_message_descriptor_for_type(".service.Request", &all_descriptors).unwrap();
        expect!(&md).to(be_equal_to(&request_msg));
        expect!(&fd).to(be_equal_to(&request_file));

        // relative name
        let (md, fd) =
            find_message_descriptor_for_type("AnotherRequest", &all_descriptors).unwrap();
        ...
        expect!(error_msg.starts_with(
            "Did not find a message type 'MissingType' in any of the file descriptors"
        ))
        .to(be_true());
    }
    ...
    fn find_service_descriptor_for_type_test() {
        ...
        let all_descriptors = FileDescriptorSet {
            file: vec![service.clone(), service2.clone()],
        };

        let (fd, sd) =
            find_service_descriptor_for_type(".service.Service", &all_descriptors).unwrap();
        expect!(fd).to(be_equal_to(service));
        expect!(sd).to(be_equal_to(service_desc));

        let (fd, sd) =
            find_service_descriptor_for_type("RelativeNameService", &all_descriptors).unwrap();
        expect!(fd).to(be_equal_to(service2));
        expect!(sd).to(be_equal_to(relative_name_service));

        // missing package case
        let result_err =
            find_service_descriptor_for_type(".missing.MissingService", &all_descriptors);
        expect!(result_err.as_ref()).to(be_err());
        expect!(&result_err.unwrap_err().to_string()).to(be_equal_to(
            "Did not find any file descriptors for a package 'missing'",
        ));
        ...
        let result_err =
            find_service_descriptor_for_type(".service.MissingService", &all_descriptors);
        expect!(result_err.as_ref()).to(be_err());
        expect!(&result_err.unwrap_err().to_string()).to(be_equal_to(
    ...
    fn find_file_descriptors_test() {
        ...
        _check_find_file_descriptors(
            Some("service"),
            &all_descriptors_with_package_names,
            &all_descriptors,
        );
        ...
        _check_find_file_descriptors(
            Some(""),
            &all_descriptors_with_no_pacakge_names,
            &all_descriptors,
        );
    ...
    fn _check_find_file_descriptors(
        ...
        let actual = find_file_descriptors(package, all_descriptors)
            .unwrap()
            .iter()
            .map(|d: &FileDescriptorProto| d.name.clone().unwrap_or_default())
    ...
    fn find_method_descriptor_for_service_test() {
        ...
        let service_desc = ServiceDescriptorProto {
            name: Some("Service".to_string()),
            method: vec![method_desc1.clone(), method_desc2.clone()],
            ..ServiceDescriptorProto::default()
        };
        ...
        expect!(result_err.as_ref()).to(be_err());
        expect!(result_err.unwrap_err().to_string()).to(be_equal_to(
            "Did not find the method missing in the Protobuf descriptor for service 'Service'",
        ));
    }
    ...
    HJvdG8z";
        ...
        let field_descriptor = DescriptorProto {
            ...
            field: vec![key_descriptor.clone(), value_descriptor.clone()],
            options: Some(MessageOptions {
                message_set_wire_format: None,
                no_standard_descriptor_accessor: None,
                deprecated: None,
