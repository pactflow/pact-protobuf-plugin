...
use crate::message_decoder::generators::{GeneratorError, data_value_to_proto_value};
...
impl Default for PactCodec {
    fn default() -> Self {
        panic!(
            "Default called for PactCodec, but it requires a service descriptor and Pact message"
        )
    }
}
...
impl DynamicMessage {
    ...
    pub fn new(field_data: &[ProtobufField], descriptors: &FileDescriptorSet) -> DynamicMessage {
        ...
        self.fields
            ...
            .flat_map(|f| {
                ...
                if f.repeated_field() && !f.additional_data.is_empty() {
                    result.extend(f.additional_data.iter().map(|d| f.clone_with_data(d)));
                }
                result
            })
            .collect()
    ...
    pub fn write_to<B>(&self, buffer: &mut B) -> anyhow::Result<()>
    where
        B: BufMut,
    {
        for (field_num, field) in self
            .fields
            .iter()
        ...
        B: BufMut,
    {
        ...
        match field.wire_type {
            ...
            WireType::SixtyFourBit => match data {
                ...
                _ => {
                    return Err(anyhow!(
                        "Expected a 64 bit value, but field is {}",
                        field.data
                    ));
                }
            },
            WireType::LengthDelimited => match data {
                ...
                _ => {
                    return Err(anyhow!(
                        "Expected a length delimited value, but field is {}",
                        field.data
                    ));
                }
            },
            WireType::ThirtyTwoBit => match data {
                ...
                _ => {
                    return Err(anyhow!(
                        "Expected a 32 bit value, but field is {}",
                        field.data
                    ));
                }
            },
    ...
    pub fn set_field_value(
        ...
        if let Some(PathToken::Root) = iter.peek() {
                ...
                if let Some(PathToken::Index(index)) = segment {
                    if index == 0 {
                        ...
                        if additional_index >= v.additional_data.len() {
                            v.additional_data
                                .resize(additional_index + 1, v.data.clone());
                        }
            ...
            Err(anyhow!(
                "Path '{}' does not start with a root path marker ('$')",
                path
            ))
        }
    }
    ...
    where
        F: FnOnce(&mut ProtobufField, Option<PathToken>),
    {
        ...
        if let Some(next) = path_tokens.next() {
            match next {
                ...
                PathToken::Field(name) => {
                    return if let Some(field) = find_field_value(fields, name.as_str()) {
                        if path_tokens.peek().is_none() {
                            callback(field, None);
                            Ok(())
                        } else {
                            match &mut field.data {
                                ProtobufFieldData::Enum(_, _) => Err(anyhow!(
                                    "Support for dynamically fetching enum values is not supported yet"
                                )),
                                ...
                                _ => match path_tokens.next() {
                                    Some(PathToken::Star) | Some(PathToken::StarIndex) => {
                                        if path_tokens.peek().is_none() {
                                            ...
                                            Err(anyhow!(
                                                "Path does not match any field in the message (additional path \
                    segments can only be applied to a child message, but field type is '{}')",
                                                field.data.type_name()
                                            ))
                                        }
                                    }
                                    Some(PathToken::Index(index)) => {
                                        if field.repeated_field() && path_tokens.peek().is_none() {
                                            callback(field, Some(PathToken::Index(*index)));
                                            Ok(())
                                        } else {
                                            Err(anyhow!(
                                                "Path segment '{}' can only be applied to repeated fields",
                                                index
                                            ))
                                        }
                                    }
                                    Some(segment) => Err(anyhow!(
                                        "Path segment '{}' can not be applied any field in the message",
                                        segment
                                    )),
                                    None => Err(anyhow!(
                                        "Path name '{}' does not match any field in the message",
                                        name
                                    )),
                                },
                            }
                ...
                PathToken::Index(_) => {
                    return Err(anyhow!("Support for index paths is not supported yet"));
                }
                PathToken::Star => {
                    return Err(anyhow!("Support for '*' in paths is not supported yet"));
                }
                PathToken::StarIndex => {
                    return Err(anyhow!("Support for '[*]' in paths is not supported yet"));
                }
            ...
            return Err(anyhow!(
                "Path does not match any field in the message (end of path tokens reached)"
            ));
        }
                ...
                if let Some(value) = &value {
                    if generator.corresponds_to_mode(mode) {
                        match generator.generate_value(&value.data, &context, &vm_boxed) {
                            ...
                            Err(err) => {
                                warn!(
                                    "Failed to apply generator '{}' for field {}: {}",
                                    path, value, err
                                );
                                if let Some(GeneratorError::ProviderStateValueIsCollection(val)) =
                                    err.downcast_ref::<GeneratorError>()
                                {
                                    if value.repeated_field() && val.wrapped.is_array() {
                                        let array = as_array(val)?;
                                        trace!(
                                            "Applying a array value ({} items) to repeated field '{}'",
                                            array.len(),
                                            value.field_name
                                        );
...
impl Decoder for DynamicMessageDecoder {
    ...
    fn decode(&mut self, src: &mut DecodeBuf<'_>) -> Result<Option<Self::Item>, Self::Error> {
        ...
        match decode_message(src, &self.descriptor, &self.file_descriptor_set) {
            Ok(fields) => Ok(Some(DynamicMessage::new(
                fields.as_slice(),
            ...
            Err(err) => {
                ...
                Err(Status::invalid_argument(format!(
                    "Failed to decode the message - {err}"
                )))
            }
        }
    }
...
mod tests {
    ...
    fn dynamic_message_fetch_value_with_no_fields() {
        let fields = vec![];
        let descriptors = FileDescriptorSet { file: vec![] };
    ...
    fn dynamic_message_fetch_value_with_matching_child_field() {
        ...
        let child_descriptor = DescriptorProto {
            ...
            field: vec![child_proto_1.clone(), child_proto_2.clone()],
            extension: vec![],
            nested_type: vec![],
            enum_type: vec![],
            extension_range: vec![],
    ...
    fn dynamic_message_generate_value_with_no_fields() {
        ...
        expect!(message.apply_generators(
            Some(&generators),
    ...
    fn dynamic_message_generate_value_with_matching_child_field() {
        ...
        expect!(
            message
                .fetch_field_value(&path)
                .unwrap()
                .data
                .as_i64()
                .unwrap()
        )
        .to_not(be_equal_to(100));
    }
    ...
    fn dynamic_message_inject_array_into_non_repeated_field() {
        ...
        let result =
            message.apply_generators(Some(&generators), &GeneratorTestMode::Provider, &context);
    ...
    fn dynamic_message_write_to_test() {
        ...
        let descriptor = DescriptorProto {
            field: vec![field_descriptor.clone()],
            ..DescriptorProto::default()
        };
    ...
    fn dynamic_message_write_to_test_with_child_field() {
        ...
        let descriptors = FileDescriptorSet {
            file: vec![FileDescriptorProto {
                message_type: vec![descriptor.clone(), child_descriptor.clone()],
    ...
    fn dynamic_message_write_to_test_with_repeated_fields() {
        ...
        expect!(result).to(be_equal_to(vec![
            field_1_1, field_1_2, field_1_3, field_2, field_3,
        ]));
    }
    ...
    fn dynamic_message_write_to_test_with_repeated_field_with_additional_values() {
        ...
        let field_1 = ProtobufField {
            ...
            data: ProtobufFieldData::Integer64(100),
            additional_data: vec![
                ProtobufFieldData::Integer64(101),
                ProtobufFieldData::Integer64(102),
            ],
            descriptor: field_descriptor_1.clone(),
        };
        ...
        assert_eq!(
            result,
            vec![field_1_1, field_1_2, field_1_3, field_2, field_3]
        );
    }
}
