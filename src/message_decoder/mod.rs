...
use prost::encoding::{WireType, decode_key, decode_varint, encode_varint};
use prost_types::field_descriptor_proto::Type;
use prost_types::{DescriptorProto, EnumDescriptorProto, FieldDescriptorProto, FileDescriptorSet};
use tracing::{debug, error, trace, warn};

use crate::utils::{
    as_hex, find_enum_by_name, find_enum_by_name_in_message, find_message_descriptor_for_type,
    is_repeated_field, last_name, should_be_packed_type,
};
...
impl ProtobufField {
    ...
    pub fn default_field(
        ...
        default_field_data(field_descriptor, descriptor, fds).map(|data| ProtobufField {
        ...
        match &descriptor.default_value {
            Some(s) => {
                ...
                match self {
                    ...
                    ProtobufFieldData::UInteger32(_) => {
                        ProtobufFieldData::UInteger32(s.parse().unwrap_or_default())
                    }
                    ProtobufFieldData::Integer32(_) => {
                        ProtobufFieldData::Integer32(s.parse().unwrap_or_default())
                    }
                    ProtobufFieldData::UInteger64(_) => {
                        ProtobufFieldData::UInteger64(s.parse().unwrap_or_default())
                    }
                    ProtobufFieldData::Integer64(_) => {
                        ProtobufFieldData::Integer64(s.parse().unwrap_or_default())
                    }
                    ProtobufFieldData::Float(_) => {
                        ProtobufFieldData::Float(s.parse().unwrap_or_default())
                    }
                    ProtobufFieldData::Double(_) => {
                        ProtobufFieldData::Double(s.parse().unwrap_or_default())
                    }
                    ProtobufFieldData::Bytes(_) => ProtobufFieldData::Bytes(s.as_bytes().to_vec()),
                    ProtobufFieldData::Enum(_, descriptor) => {
                        ProtobufFieldData::Enum(s.parse().unwrap_or_default(), descriptor.clone())
                    }
                    ProtobufFieldData::Message(_, descriptor) => {
                        ProtobufFieldData::Message(Default::default(), descriptor.clone())
                    }
            ...
            None => {
                ...
                match self {
                    ...
                    ProtobufFieldData::Enum(_, descriptor) => {
                        ProtobufFieldData::Enum(0, descriptor.clone())
                    }
                    ProtobufFieldData::Message(_, descriptor) => {
                        ProtobufFieldData::Message(Default::default(), descriptor.clone())
                    }
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ProtobufFieldData::Enum(n, descriptor) => {
                let enum_value_name = descriptor
                    .value
                    .iter()
    ...
    B: Buf,
{
    ...
    trace!(
        "all descriptors available for decoding the message: {:?}",
        descriptors
    );
    ...
    while buffer.has_remaining() {
        ...
        trace!(
            field_num,
            ?wire_type,
            "read field header, bytes remaining = {}",
            buffer.remaining()
        );
            ...
            Ok(field_descriptor) => {
                ...
                let data = match wire_type {
                    WireType::Varint => {
                        ...
                        match t {
                            Type::Int64 => {
                                vec![(ProtobufFieldData::Integer64(varint as i64), wire_type)]
                            }
                            Type::Uint64 => {
                                vec![(ProtobufFieldData::UInteger64(varint), wire_type)]
                            }
                            Type::Int32 => {
                                vec![(ProtobufFieldData::Integer32(varint as i32), wire_type)]
                            }
                            Type::Bool => vec![(ProtobufFieldData::Boolean(varint > 0), wire_type)],
                            Type::Uint32 => {
                                vec![(ProtobufFieldData::UInteger32(varint as u32), wire_type)]
                            }
                            Type::Enum => {
                                vec![(
                                    decode_enum(
                            ...
                            Type::Sint64 => vec![(
                                ProtobufFieldData::Integer64(
                                    ((varint >> 1) as i64) ^ (-((varint & 1) as i64)),
                                ),
                                wire_type,
                            )],
                            _ => {
                    ...
                    WireType::SixtyFourBit => {
                        ...
                        match t {
                            Type::Double => {
                                vec![(ProtobufFieldData::Double(buffer.get_f64_le()), wire_type)]
                            }
                            Type::Fixed64 => vec![(
                                ProtobufFieldData::UInteger64(buffer.get_u64_le()),
                                wire_type,
                            )],
                            Type::Sfixed64 => {
                                vec![(ProtobufFieldData::Integer64(buffer.get_i64_le()), wire_type)]
                            }
                            _ => {
                    ...
                    WireType::LengthDelimited => {
                        ...
                        let mut data_buffer = if buffer.remaining() >= data_length as usize {
                            ...
                            return Err(anyhow!(
                                "Insufficient data remaining ({} bytes) to read {} bytes for field {}",
                                buffer.remaining(),
                                data_length,
                                field_num
                            ));
                        ...
                        match t {
                            Type::String => vec![(
                                ProtobufFieldData::String(from_utf8(&data_buffer)?.to_string()),
                                wire_type,
                            )],
                            Type::Message => {
                ...
                .or_else(|_| {
                  descriptor.nested_type.iter().find(
                    |message_descriptor| message_descriptor.name.as_deref() == Some(last_name(full_type_name))
                            ...
                            Type::Bytes => {
                                vec![(ProtobufFieldData::Bytes(data_buffer.to_vec()), wire_type)]
                            }
                            _ => {
                                if should_be_packed_type(t) && is_repeated_field(&field_descriptor)
                                {
                                    debug!(
                                        "Reading length delimited field as a packed repeated field"
                                    );
                                    decode_packed_field(
                                        field_descriptor,
                                        descriptor,
                                    ...
                                    error!(
                                        "Was expecting {:?} but received an unknown length-delimited type",
                                        t
                                    );
                                    let mut buf =
                                        BytesMut::with_capacity((data_length + 8) as usize);
                    ...
                    WireType::ThirtyTwoBit => {
                        ...
                        match t {
                            Type::Float => {
                                vec![(ProtobufFieldData::Float(buffer.get_f32_le()), wire_type)]
                            }
                            Type::Fixed32 => vec![(
                                ProtobufFieldData::UInteger32(buffer.get_u32_le()),
                                wire_type,
                            )],
                            Type::Sfixed32 => {
                                vec![(ProtobufFieldData::Integer32(buffer.get_i32_le()), wire_type)]
                            }
                            _ => {
                                error!(
                                    "Was expecting {:?} but received an unknown fixed 32 bit type",
                                    t
                                );
                    ...
                    _ => {
                        return Err(anyhow!(
                            "Messages with {:?} wire type fields are not supported",
                            wire_type
                        ));
                    }
                };
...
fn decode_enum(
    ...
    let enum_proto = find_enum_by_name_in_message(&descriptor.enum_type, enum_type_name.as_str())
        ...
        .ok_or_else(|| {
            anyhow!(
                "Did not find the enum {} for the field in the Protobuf descriptor",
                enum_type_name
            )
        })?;
...
fn decode_packed_field(
    ...
    match t {
        Type::Double => {
            while data.remaining() >= mem::size_of::<f64>() {
                values.push((
                    ProtobufFieldData::Double(data.get_f64_le()),
                    WireType::SixtyFourBit,
                ));
            }
        }
        Type::Float => {
            while data.remaining() >= mem::size_of::<f32>() {
                values.push((
                    ProtobufFieldData::Float(data.get_f32_le()),
                    WireType::ThirtyTwoBit,
                ));
            }
        }
        Type::Int64 => {
            while data.remaining() > 0 {
                ...
                values.push((
                    ProtobufFieldData::Integer64(varint as i64),
                    WireType::Varint,
                ));
            }
        }
        ...
        Type::Int32 => {
            while data.remaining() > 0 {
                let varint = decode_varint(data)?;
                values.push((
                    ProtobufFieldData::Integer32(varint as i32),
                    WireType::Varint,
                ));
            }
        }
        ...
        Type::Fixed64 => {
            while data.remaining() >= mem::size_of::<u64>() {
                values.push((
                    ProtobufFieldData::UInteger64(data.get_u64_le()),
                    WireType::SixtyFourBit,
                ));
            }
        }
        Type::Fixed32 => {
            while data.remaining() >= mem::size_of::<u32>() {
                values.push((
                    ProtobufFieldData::UInteger32(data.get_u32_le()),
                    WireType::ThirtyTwoBit,
                ));
            }
        }
        Type::Uint32 => {
            while data.remaining() > 0 {
                let varint = decode_varint(data)?;
                values.push((
                    ProtobufFieldData::UInteger32(varint as u32),
                    WireType::Varint,
                ));
            }
        }
        Type::Sfixed32 => {
            while data.remaining() >= mem::size_of::<i32>() {
                values.push((
                    ProtobufFieldData::Integer32(data.get_i32_le()),
                    WireType::ThirtyTwoBit,
                ));
            }
        }
        Type::Sfixed64 => {
            while data.remaining() >= mem::size_of::<i64>() {
                values.push((
                    ProtobufFieldData::Integer64(data.get_i64_le()),
                    WireType::SixtyFourBit,
                ));
            }
        }
        ...
        Type::Sint64 => {
            while data.remaining() > 0 {
                let varint = decode_varint(data)?;
                values.push((
                    ProtobufFieldData::Integer64(((varint >> 1) as i64) ^ (-((varint & 1) as i64))),
                    WireType::Varint,
                ));
            }
        }
    ...
    if data.is_empty() {
        ...
        Err(anyhow!(
            "Failed to decode packed repeated field, there was still {} bytes in the buffer",
            data.remaining()
        ))
    }
}
...
fn find_field_descriptor(
    ...
    descriptor
        ...
        .ok_or_else(|| {
            anyhow!(
                "Did not find a field with number {} in the descriptor",
                field_num
            )
        })
}
...
mod tests {
    ...
    use prost_types::{
        DescriptorProto, EnumDescriptorProto, EnumValueDescriptorProto, FileDescriptorSet,
    };

    use crate::message_builder::tests::REPEATED_ENUM_DESCRIPTORS;
    use crate::message_decoder::{ProtobufFieldData, decode_message};
    use crate::protobuf::tests::DESCRIPTOR_WITH_ENUM_BYTES;
    use crate::{
        bool_field_descriptor, bytes_field_descriptor, enum_field_descriptor, f32_field_descriptor,
        f64_field_descriptor, i32_field_descriptor, i64_field_descriptor, message_field_descriptor,
        string_field_descriptor, u32_field_descriptor, u64_field_descriptor,
    };
    ...
    fn decode_string() {
        ...
        let descriptor = DescriptorProto {
            name: Some("TestMessage".to_string()),
            field: vec![string_field_descriptor!("type", 1)],
            extension: vec![],
            nested_type: vec![],
            enum_type: vec![],
            extension_range: vec![],
        ...
        expect!(&field_result.data).to(be_equal_to(&ProtobufFieldData::String(
            str_data.to_string(),
        )));
    }
    ...
    fn decode_message_test() {
        ...
        let message_descriptor = DescriptorProto {
            ...
            field: vec![field1.clone(), field2.clone()],
            extension: vec![],
            nested_type: vec![],
            enum_type: vec![],
            extension_range: vec![],
        ...
        let descriptor = DescriptorProto {
            name: Some("TestMessage".to_string()),
            field: vec![message_field_descriptor!("message", 1, "InitPluginRequest")],
            extension: vec![],
            nested_type: vec![message_descriptor.clone()],
            enum_type: vec![],
            extension_range: vec![],
            oneof_decl: vec![],
            options: None,
        ...
        expect!(field_result.field_num).to(be_equal_to(1));
        expect!(field_result.wire_type).to(be_equal_to(WireType::LengthDelimited));
        expect!(&field_result.data).to(be_equal_to(&ProtobufFieldData::Message(
            encoded,
            message_descriptor,
        )));
    }
    ...
    fn default_field_value_test_boolean() {
        ...
        expect!(ProtobufFieldData::Boolean(true).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Boolean(false)));
        ...
        expect!(ProtobufFieldData::Boolean(true).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Boolean(true)));
    ...
    fn default_field_value_test_string() {
        ...
        expect!(ProtobufFieldData::String("true".to_string()).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::String("".to_string())));

        let descriptor = prost_types::FieldDescriptorProto {
            default_value: Some("true".to_string()),
            ..string_field_descriptor!("field", 1)
        };
        expect!(ProtobufFieldData::String("other".to_string()).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::String("true".to_string())));
    }

    #[test]
    fn default_field_value_test_u32() {
        let descriptor = u32_field_descriptor!("field", 1);
        expect!(ProtobufFieldData::UInteger32(123).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::UInteger32(0)));
        ...
        expect!(ProtobufFieldData::UInteger32(123).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::UInteger32(100)));
    ...
    fn default_field_value_test_i32() {
        ...
        expect!(ProtobufFieldData::Integer32(123).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Integer32(0)));
        ...
        expect!(ProtobufFieldData::Integer32(123).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Integer32(100)));
    ...
    fn default_field_value_test_u64() {
        ...
        expect!(ProtobufFieldData::UInteger64(123).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::UInteger64(0)));
        ...
        expect!(ProtobufFieldData::UInteger64(123).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::UInteger64(100)));
    ...
    fn default_field_value_test_i64() {
        ...
        expect!(ProtobufFieldData::Integer64(123).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Integer64(0)));
        ...
        expect!(ProtobufFieldData::Integer64(123).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Integer64(100)));
    ...
    fn default_field_value_test_f32() {
        ...
        expect!(ProtobufFieldData::Float(123.0).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Float(0.0)));
        ...
        expect!(ProtobufFieldData::Float(123.0).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Float(100.0)));
    ...
    fn default_field_value_test_f64() {
        ...
        expect!(ProtobufFieldData::Double(123.0).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Double(0.0)));
        ...
        expect!(ProtobufFieldData::Double(123.0).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Double(100.0)));
    ...
    fn default_field_value_test_enum() {
        ...
        expect!(
            ProtobufFieldData::Enum(2, enum_descriptor.clone()).default_field_value(&descriptor)
        )
        .to(be_equal_to(ProtobufFieldData::Enum(
            0,
            enum_descriptor.clone(),
        )));
    ...
    fn default_field_value_test_bytes() {
        ...
        expect!(ProtobufFieldData::Bytes(vec![1, 2, 3, 4]).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Bytes(vec![])));

        let descriptor = prost_types::FieldDescriptorProto {
            default_value: Some("true".to_string()),
            ..bytes_field_descriptor!("field", 1)
        };
        expect!(ProtobufFieldData::Bytes(vec![1, 2, 3, 4]).default_field_value(&descriptor)).to(
            be_equal_to(ProtobufFieldData::Bytes(vec![116, 114, 117, 101])),
        );
    }
    ...
    fn default_field_value_test_message() {
        ...
        expect!(
            ProtobufFieldData::Message(vec![1, 2, 3, 4], message_descriptor.clone())
                .default_field_value(&descriptor)
        )
        .to(be_equal_to(ProtobufFieldData::Message(
            vec![],
    ...
    fn default_field_value_test_unknown() {
        let descriptor = bytes_field_descriptor!("field", 1);
        expect!(ProtobufFieldData::Unknown(vec![1, 2, 3, 4]).default_field_value(&descriptor))
            .to(be_equal_to(ProtobufFieldData::Unknown(vec![])));
    ...
    fn decode_message_with_global_enum_field() {
        ...
        expect!(field_result.field_num).to(be_equal_to(5));
        expect!(field_result.wire_type).to(be_equal_to(WireType::Varint));
        expect!(&field_result.data)
            .to(be_equal_to(&ProtobufFieldData::Enum(1, enum_proto.clone())));
    }
    ...
    fn decode_message_with_repeated_enum_field() {
        ...
        let message_descriptor = main_descriptor
            .message_type
            .iter()
            .find(|md| md.name.clone().unwrap_or_default() == "BrokenSampleRequest")
            .unwrap();
    ...
    XNwb25zZWIGcHJvdG8z";
        ...
        let message_descriptor = main_descriptor
            .message_type
            .iter()
            .find(|md| md.name.clone().unwrap_or_default() == "GetUserResponse")
            .unwrap();

        // buf: b"\n\x041234\x12\x0cElla Streich\x1a\x04Ella:\x07StreichB\x14Ella.Streich@test.ioH\x01", len: 59
        let message_bytes: &[u8] = &[
            10, 4, 49, 50, 51, 52, 18, 12, 69, 108, 108, 97, 32, 83, 116, 114, 101, 105, 99, 104,
            26, 4, 69, 108, 108, 97, 58, 7, 83, 116, 114, 101, 105, 99, 104, 66, 20, 69, 108, 108,
            97, 46, 83, 116, 114, 101, 105, 99, 104, 64, 116, 101, 115, 116, 46, 105, 111, 72, 1,
        ];
        let mut buffer = Bytes::from(message_bytes);
        let result = decode_message(&mut buffer, &message_descriptor, &fds).unwrap();
        ...
        expect!(first_field.data.clone())
            .to(be_equal_to(ProtobufFieldData::String("1234".to_string())));

        let field = &result[1];
        expect!(field.field_num).to(be_equal_to(2));
        expect!(field.wire_type).to(be_equal_to(WireType::LengthDelimited));
        expect!(field.data.clone()).to(be_equal_to(ProtobufFieldData::String(
            "Ella Streich".to_string(),
        )));
        ...
        expect!(field.wire_type).to(be_equal_to(WireType::LengthDelimited));
        expect!(field.data.clone()).to(be_equal_to(ProtobufFieldData::Unknown(vec![
            20, 69, 108, 108, 97, 46, 83, 116, 114, 101, 105, 99, 104, 64, 116, 101, 115, 116, 46,
            105, 111,
        ])));
