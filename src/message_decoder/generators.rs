...
use pact_models::generators::datetime_expressions::{
    execute_date_expression, execute_datetime_expression, execute_time_expression,
};
use pact_models::generators::{
    GenerateValue, Generator, UuidFormat, VariantMatcher, generate_ascii_string, generate_decimal,
    generate_hexadecimal, generate_value_from_context,
};
        ...
        let result = match self {
            Generator::RandomInt(min, max) => {
                ...
                match value {
                    ProtobufFieldData::String(_) => {
                        Ok(ProtobufFieldData::String(rand_int.to_string()))
                    }
                    ...
                    ProtobufFieldData::Integer64(_) => {
                        Ok(ProtobufFieldData::Integer64(rand_int as i64))
                    }
                    ProtobufFieldData::Integer32(_) => Ok(ProtobufFieldData::Integer32(rand_int)),
                    ProtobufFieldData::UInteger64(_) => {
                        Ok(ProtobufFieldData::UInteger64(rand_int as u64))
                    }
                    ProtobufFieldData::UInteger32(_) => {
                        Ok(ProtobufFieldData::UInteger32(rand_int as u32))
                    }
            ...
            Generator::Uuid(format) => match value {
                ProtobufFieldData::String(_) => match format.unwrap_or_default() {
                    UuidFormat::Simple => Ok(ProtobufFieldData::String(
                        Uuid::new_v4().as_simple().to_string(),
                    )),
                    UuidFormat::LowerCaseHyphenated => Ok(ProtobufFieldData::String(
                        Uuid::new_v4().as_hyphenated().to_string(),
                    )),
                    UuidFormat::UpperCaseHyphenated => Ok(ProtobufFieldData::String(
                        Uuid::new_v4().as_hyphenated().to_string().to_uppercase(),
                    )),
                    UuidFormat::Urn => Ok(ProtobufFieldData::String(
            ...
            Generator::RandomDecimal(digits) => {
                ...
                match value {
                    ...
                    ProtobufFieldData::Integer64(_) => {
                        Ok(ProtobufFieldData::Integer64(decimal.parse()?))
                    }
                    ProtobufFieldData::Integer32(_) => {
                        Ok(ProtobufFieldData::Integer32(decimal.parse()?))
                    }
                    ProtobufFieldData::UInteger64(_) => {
                        Ok(ProtobufFieldData::UInteger64(decimal.parse()?))
                    }
                    ProtobufFieldData::UInteger32(_) => {
                        Ok(ProtobufFieldData::UInteger32(decimal.parse()?))
                    }
                    _ => Err(anyhow!(
                        "Could not generate a random decimal from {}",
                        value
                    )),
                }
            }
            Generator::RandomHexadecimal(digits) => match value {
                ProtobufFieldData::String(_) => Ok(ProtobufFieldData::String(
                    generate_hexadecimal(*digits as usize),
                )),
                _ => Err(anyhow!(
                    "Could not generate a random hexadecimal from {}",
                    value
                )),
            },
            Generator::RandomString(size) => match value {
                ProtobufFieldData::String(_) => Ok(ProtobufFieldData::String(
                    generate_ascii_string(*size as usize),
                )),
                ...
                ProtobufFieldData::String(_) => {
                    ...
                    match parser.parse(regex) {
                        Ok(hir) => {
                            ...
                            Ok(ProtobufFieldData::String(
                                rand::thread_rng().sample::<String, _>(gen_regex),
                            ))
                        }
                        Err(err) => {
                            warn!("'{}' is not a valid regular expression - {}", regex, err);
                            Err(anyhow!(
                                "Could not generate a random string from {} - {}",
                                regex,
                                err
                            ))
                        }
            ...
            Generator::Date(format, exp) => {
                ...
                let date =
                    execute_date_expression(&base, exp.clone().unwrap_or_default().as_str())?;
                let result = match format {
                    Some(pattern) => match parse_pattern(pattern) {
                        ...
                        Err(err) => {
                            ...
                            Err(anyhow!(
                                "Could not generate a random date from {} - {}",
                                pattern,
                                err
                            ))
                        }
                ...
                result.and_then(|v| match value {
                    ProtobufFieldData::String(_) => Ok(ProtobufFieldData::String(v)),
                    _ => Err(anyhow!(
                        "Can not generate a date value for a field type {:?}",
                        value
                    )),
                })
            }
            Generator::Time(format, exp) => {
                ...
                let time =
                    execute_time_expression(&base, exp.clone().unwrap_or_default().as_str())?;
                let result = match format {
                    Some(pattern) => match parse_pattern(pattern) {
                        Ok(tokens) =>
                        {
                        ...
                        Err(err) => {
                            ...
                            Err(anyhow!(
                                "Could not generate a random time from {} - {}",
                                pattern,
                                err
                            ))
                        }
                ...
                result.and_then(|v| match value {
                    ProtobufFieldData::String(_) => Ok(ProtobufFieldData::String(v)),
                    _ => Err(anyhow!(
                        "Can not generate a time value for a field type {:?}",
                        value
                    )),
                })
            }
            Generator::DateTime(format, exp) => {
                ...
                let date_time =
                    execute_datetime_expression(&base, exp.clone().unwrap_or_default().as_str())?;
                let result = match format {
                    Some(pattern) => match parse_pattern(pattern) {
                        Ok(tokens) =>
                        {
                        ...
                        Err(err) => {
                            warn!("Date format {} is not valid - {}", pattern, err);
                            Err(anyhow!(
                                "Could not generate a random date-time from {} - {}",
                                pattern,
                                err
                            ))
                        }
                ...
                result.and_then(|v| match value {
                    ProtobufFieldData::String(_) => Ok(ProtobufFieldData::String(v)),
                    _ => Err(anyhow!(
                        "Can not generate a date-time value for a field type {:?}",
                        value
                    )),
                })
            }
            Generator::RandomBoolean => {
                ...
                match value {
                    ...
                    ProtobufFieldData::UInteger32(_) => {
                        Ok(ProtobufFieldData::UInteger32(u32::from(b)))
                    }
                    ProtobufFieldData::Integer32(_) => {
                        Ok(ProtobufFieldData::Integer32(i32::from(b)))
                    }
                    ProtobufFieldData::UInteger64(_) => {
                        Ok(ProtobufFieldData::UInteger64(u64::from(b)))
                    }
                    ProtobufFieldData::Integer64(_) => {
                        Ok(ProtobufFieldData::Integer64(i64::from(b)))
                    }
                    ProtobufFieldData::Float(_) => {
                        Ok(ProtobufFieldData::Float(if b { 1.0 } else { 0.0 }))
                    }
                    ProtobufFieldData::Double(_) => {
                        Ok(ProtobufFieldData::Double(if b { 1.0 } else { 0.0 }))
                    }
                    _ => Err(anyhow!(
                        "Can not generate a boolean value for a field type {:?}",
                        value
                    )),
                }
            }
            ...
            Generator::MockServerURL(example, regex) => {
                ...
                if let Some(mock_server_details) = context.get("mockServer") {
                        ...
                        Some(mock_server_details) => {
                            match get_field_as_string("href", mock_server_details) {
                                Some(url) => match Regex::new(regex) {
                                    Ok(re) => Ok(ProtobufFieldData::String(replace_with_regex(
                                        example, url, re,
                                    ))),
                                    Err(err) => Err(anyhow!(
                                        "MockServerURL: Failed to generate value: {}",
                                        err
                                    )),
                                },
                                None => Err(anyhow!(
                                    "MockServerURL: can not generate a value as there is no mock server URL in the test context"
                                )),
                            }
                        }
                        None => Err(anyhow!(
                    ...
                    Err(anyhow!(
                        "MockServerURL: can not generate a value as there is no mock server details in the test context"
                    ))
                }
            }
            ...
            // }
            _ => Err(anyhow!(
                "Generator type {} is not currently supported for {:?}",
                self.name(),
                value
            )),
        };
...
pub fn data_value_to_proto_value(
    value: &ProtobufFieldData,
    val: &DataValue,
) -> anyhow::Result<ProtobufFieldData> {
    let val = val.clone();
    match value {
        ...
        ProtobufFieldData::UInteger32(_) => {
            Ok(ProtobufFieldData::UInteger32(u64::try_from(val)? as u32))
        }
        ProtobufFieldData::Integer32(_) => {
            Ok(ProtobufFieldData::Integer32(i64::try_from(val)? as i32))
        }
        ...
        _ => Err(anyhow!(
            "Can not generate a value from the provider state for a field type {:?}",
            value
        )),
    }
}
    ...
    fn generate_value(
        ...
        match self {
            Generator::ProviderStateGenerator(exp, dt) => {
                ...
                trace!(
                    ?exp,
                    ?provider_state_config,
                    ?dt,
                    "Calling generate_value_from_context"
                );
            ...
            _ => {
                ...
                generator
                    .generate_value(&value.value, context, matcher)
...
mod tests {
    ...
    fn generate_datetime() {
        ...
        let value = ProtobufFieldData::Enum(
            1,
            EnumDescriptorProto {
                name: None,
                value: vec![],
        ...
        let value = ProtobufFieldData::Message(
            vec![],
            DescriptorProto {
                name: None,
                field: vec![],
    ...
    fn generate_uuid() {
        ...
        let s = result.unwrap().to_string();
        let re =
            Regex::new("urn:uuid:[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}")
                .unwrap();
    ...
    fn generate_provider_state() {
        ...
        let result = generator
            .generate_value(&value, &provider_state2, &vm)
            .unwrap();
        let s = result.to_string();
