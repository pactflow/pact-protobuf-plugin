...
impl Protoc {
    ...
    async fn invoke(&self) -> anyhow::Result<String> {
        ...
        match Command::new(&self.protoc_path)
            .arg("--version")
            .output()
            .await
        {
            Ok(out) => {
                if out.status.success() {
                    ...
                    error!(
                        "Protoc output: {}",
                        from_utf8(out.stdout.as_slice()).unwrap_or_default()
                    );
                    error!(
                        "Protoc stderr: {}",
                        from_utf8(out.stderr.as_slice()).unwrap_or_default()
                    );
                    Err(anyhow!(
                        "Failed to invoke protoc binary: exit code {}",
                        out.status
                    ))
                }
            }
    ...
    pub(crate) async fn parse_proto_file(
        &self,
        proto_file: &Path,
...
pub(crate) async fn setup_protoc(
...
async fn download_protoc(
    ...
    let url = base_url.join(
        format!(
            "v{}/protoc-{}-{}.zip",
            protoc_version, protoc_version, os_type
        )
        .as_str(),
    )?;
...
async fn local_protoc(os_info: &Info, additional_includes: &[String]) -> anyhow::Result<Protoc> {
    let path = PathBuf::from(".").join("protoc").join("bin");
...
mod tests {
    ...
    fn os_type_test() {
        ...
        expect!(os_type(Bitness::X64, "aarch64", "linux").as_str())
            .to(be_equal_to("linux-aarch_64"));
