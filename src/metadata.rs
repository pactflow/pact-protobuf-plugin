...
use pact_matching::{CoreMatchingContext, MatchingContext, Mismatch, matchers};
use pact_models::generators::Generator;
use pact_models::json_utils::json_to_string;
use pact_models::matchingrules::expressions::{
    MatchingRuleDefinition, is_matcher_def, parse_matcher_def,
};
...
impl MessageMetadataValue {
    ...
    pub fn new<S: Into<String>>(s: S) -> Self {
        MessageMetadataValue { value: s.into() }
    }
}
...
pub fn process_metadata(
    metadata_config: Option<&Value>,
) -> anyhow::Result<Option<MessageMetadata>> {
                    ...
                    if !mrd.rules.is_empty() {
                        for rule in &mrd.rules {
                            match rule {
                                ...
                                Either::Right(mr) => {
                                    return Err(anyhow!(
                                        "Was expecting a value for metadata key '{}', but got a matching reference {:?}",
                                        key,
                                        mr
                                    ));
                                }
...
pub fn compare_metadata(
    ...
    if expected_metadata.is_empty() {
        ...
        let mismatches = expected_metadata
            .iter()
    ...
    match actual.to_str() {
        Ok(actual) => {
            if context.matcher_is_defined(&path) {
                ...
                let result = if let Err(errors) =
                    matchers::match_values(&path, &matchers, &expected, &actual.to_string())
                {
                    for mismatch in errors {
                        mismatches.push(Mismatch::MetadataMismatch {
                            ...
                            mismatch: format!(
                                "Comparison of metadata key '{}' failed: {}",
                                key, mismatch
                            ),
                        });
                    }
              ...
              mismatch: format!("Comparison of metadata key '{}' failed: expected {} but received {}", key,
                    ...
                    format!(
                        "        key '{}' with value '{}' [{}]",
                        bold.paint(key),
                        bold.paint(code_desc(&expected_status.code())),
                        result
                    )
                } else {
                    format!(
        ...
        Err(err) => {
            mismatches.push(Mismatch::MetadataMismatch {
                ...
                actual: "".to_string(),
                mismatch: format!(
                    "Could not convert actual value with key '{}' to a string - {}",
                    key, err
                ),
            });
...
fn matching_rule_description(rule: &MatchingRule) -> String {
    match rule {
        ...
        MatchingRule::MinMaxType(min, max) => {
            format!("type with length between {} and {}", min, max)
        }
...
mod tests {
    ...
    use crate::metadata::{MessageMetadataValue, compare_metadata, grpc_status, process_metadata};
    ...
    fn process_metadata_handles_any_matching_rules_in_the_values() {
        ...
        expect!(&rules_list.rules).to(be_equal_to(&vec![MatchingRule::Boolean]));
    }
    ...
    fn compare_metadata_using_matchers() {
        ...
        .to(be_equal_to(vec!["x-b".to_string()]));
    }
