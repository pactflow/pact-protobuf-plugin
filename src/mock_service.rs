...
impl MockService {
    ...
    pub(crate) async fn handle_message(
        ...
        let expected_message = decode_message(
            &mut expected_message_bytes,
            &message_descriptor,
            ...
            .map(|pd| {
                (
                    ...
                    PluginInteractionConfig {
                        ...
                        interaction_configuration: self
                            .message
                            .plugin_config
                            .get(pd.name.as_str())
                            .cloned()
        ...
        let md_context = CoreMatchingContext::new(
            DiffConfig::NoUnexpectedKeys,
            &self
                .message
                .request
        ...
        let md_mismatches = compare_metadata(
            ...
            (Ok(result), Ok((md_result, _))) => {
                {
                    ...
                    let method_name = self
                        ...
                        .name
                        .clone()
                        .unwrap_or_else(|| "unknown method".into());
                    let key =
                        match build_grpc_route(self.service_name.as_str(), method_name.as_str()) {
                            Ok(k) => k,
                    ...
                    if let Some((_, results)) = guard.get_mut(self.server_key.as_str()) {
                        ...
                        trace!(
                            store_length = route_results.1.len(),
                            "Adding result to mock server '{}' static store", self.server_key
                        );
                        route_results.0 += 1;
                        route_results.1.push((result.clone(), md_result.clone()));
                    } else {
                        error!(
                            "INTERNAL ERROR: Did not find an entry for '{}' in mock server static store",
                            self.server_key
                        );
                    }
                }
                ...
                if result.all_matched() && md_result.all_matched() {
                    ...
                    let response_contents =
                        self.message.response.first().cloned().unwrap_or_default();
                    // check for a gRPC status on the response metadata
                    if let Some(status) = grpc_status(&response_contents) {
                        info!(
                            "a gRPC status {} is set for the response, returning that",
                            status
                        );
                        Err(status)
                    } else {
                        ...
                        let mut response_bytes =
                            response_contents.contents.value().unwrap_or_default();
                        trace!("Response message has {} bytes", response_bytes.len());
                        let response_message_fields = decode_message(
                            &mut response_bytes,
                            &response_descriptor,
                            &self.file_descriptor_set,
                        )
                        ...
                        self.apply_generators(&mut message, &response_contents)
                            .map_err(|err| {
                    ...
                    Err(Status::failed_precondition(format!(
                        "Failed to match the request message - {result:?}"
                    )))
                }
            }
            (Err(err), _) => {
        ...
        for (key, value) in &response_contents.metadata {
            ...
            if key != "content-type" && key != "contenttype" && key != "grpc-status" {
                match json_to_string(value).parse() {
                    Ok(parsed_val) => match md.entry(key.as_str()) {
                        ...
                        Err(err) => {
                            error!(
                                "'{}' is not a valid gRPC metadata key, ignoring it - {}",
                                key, err
                            );
                        }
                    },
                    Err(err) => {
                        error!(
                            "'{}' is not a valid gRPC metadata value, ignoring it - {}",
                            value, err
                        );
                    }
                }
...
mod tests {
    ...
    IgBCHFoXaW8ucGFjdC9hcmVhX2NhbGN1bGF0b3LQAgFiBnByb3RvMw==";
        ...
        let ac_desc = file_descriptor_set
            .file
            .iter()
            .find(|ds| ds.name.clone().unwrap_or_default() == "area_calculator.proto")
            .cloned()
        ...
        let request_bytes: &[u8] = [
            10, 12, 18, 10, 13, 0, 0, 64, 64, 21, 0, 0, 128, 64, 10, 7, 10, 5, 13, 0, 0, 64, 64,
        ]
        .as_slice();
        let pact_json = json!({
          "interactions": [
        ...
        md.insert(
            MetadataKey::from_static("contenttype"),
            MetadataValue::from_static("application/protobuf;message=.area_calculator.AreaRequest"),
        );
        let response = mock_service
            .handle_message(request, input_message.clone(), output_message.clone(), md)
            .await;
