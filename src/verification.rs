...
use crate::metadata::{MetadataMatchResult, compare_metadata, grpc_status};
...
pub async fn verify_interaction(
    ...
    let (input_message_desc, _) =
        find_message_descriptor_for_type(input_message_name.as_str(), &all_file_descriptors)?;
    ...
    let (output_message_desc, _) =
        find_message_descriptor_for_type(output_message_name.as_str(), &all_file_descriptors)?;
    let bold = Style::new().bold();

    match build_grpc_request(
        request_body,
        metadata,
        ...
        .await
        {
            Ok(response) => {
                ...
                let (result, verification_output) = verify_response(
                    body,
                    response_metadata,
                    interaction,
                    &all_file_descriptors,
                    &method_desc,
                    &expectations.unwrap_or_default(),
                )?;
                ...
                let mut output = vec![
                    ...
                    format!(
                        ...
                        bold.paint(input_message_name)
                    ),
                    format!(
                        "    will return an output {} message [{}]",
                        bold.paint(output_message_name),
                        status_result
                    ),
                ];
            ...
            Err(err) => {
                ...
                if let Some(received_status) = err.downcast_ref::<GrpcError>() {
                    ...
                    let expected_response =
                        interaction.response.first().unwrap_or(&default_contents);
                    if let Some(expected_status) = grpc_status(expected_response) {
                        let (result, verification_output) = verify_error_response(
                            expected_response,
                        ...
                        let mut output = vec![
                            ...
                            format!(
                                ...
                                bold.paint(expected_status.code().to_string()),
                                status_result
                            ),
                        ];
                        output.extend(verification_output);
...
fn expectations_from_json_inner(path: &DocPath, acc: &mut HashMap<DocPath, String>, json: &Value) {
    match json {
        Value::Array(array) => {
            ...
            Ok(result) => {
                ...
                match result {
                    ...
                    BodyMatchResult::BodyTypeMismatch { message, .. } => {
                        results.push(VerificationMismatchResult::Error {
                            error: message,
...
fn verify_metadata(
    ...
    let context = CoreMatchingContext::new(DiffConfig::AllowUnexpectedKeys, &rules, &plugin_config);
...
async fn make_grpc_request(
    ...
    let port = json_to_num(config.get("port").cloned()).unwrap_or(8080);
    let dest = format!("http://{}:{}", host, port);

    let request_path_data = metadata.get("request-path").ok_or_else(|| {
        anyhow!("INTERNAL ERROR: request-path is not set in the request metadata")
    })?;
    let request_path = match &request_path_data.value {
        Some(data) => match data {
            proto::metadata_value::Value::NonBinaryValue(value) => {
                proto_value_to_string(value).unwrap_or_default()
            }
            _ => {
                return Err(anyhow!(
                    "INTERNAL ERROR: request-path is not set correctly in the request metadata"
                ));
            }
        },
        None => {
            return Err(anyhow!(
                "INTERNAL ERROR: request-path is not set in the request metadata"
            ));
        }
    };
    ...
    grpc.unary(request, path, codec).await.map_err(|err| {
                ...
                match value {
                    proto::metadata_value::Value::NonBinaryValue(value) => {
                        ...
                        match str_value.parse::<MetadataValue<Ascii>>() {
                            ...
                            Err(err) => {
                                warn!(
                                    "Could not parse Protobuf metadata value for key '{}' - {}",
                                    key, err
                                );
                            }
                        }
                    }
                    proto::metadata_value::Value::BinaryValue(value) => {
                        match key.parse::<MetadataKey<Binary>>() {
                            Ok(key) => {
