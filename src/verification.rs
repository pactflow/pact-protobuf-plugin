...
use crate::utils::{
    find_message_type_by_name, last_name, lookup_service_descriptors_for_interaction,
};
...
pub async fn verify_interaction(
    ...
    let (file_desc, service_desc, method_desc, _) =
    ...
    let input_message =
        find_message_type_by_name(last_name(input_message_name.as_str()), &file_desc)?.0;
    let output_message_name = method_desc.output_type.clone().unwrap_or_default();
    let output_message =
        find_message_type_by_name(last_name(output_message_name.as_str()), &file_desc)?.0;
    debug!(
        "Verifying interaction with input message '{}' and output message '{}'",
        input_message_name, output_message_name
    );
