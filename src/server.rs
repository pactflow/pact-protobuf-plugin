...
use pact_models::generators::{
    GenerateValue, Generator, GeneratorCategory, GeneratorTestMode, VariantMatcher,
};
...
use pact_plugin_driver::proto::{
    Body, CompareContentsRequest, CompareContentsResponse, ConfigureInteractionResponse,
    GenerateContentRequest, GenerateContentResponse, MetadataValue, MockServerResult,
    PluginConfiguration, StartMockServerResponse, VerificationPreparationResponse,
    VerifyInteractionResponse, body,
};
use pact_plugin_driver::utils::{
    proto_struct_to_json, proto_struct_to_map, proto_value_to_json, proto_value_to_string,
    to_proto_value,
};
...
use crate::message_decoder::{ProtobufField, decode_message};
...
use crate::utils::{
    build_grpc_route, find_message_descriptor_for_type, get_descriptors_for_interaction,
    lookup_interaction_by_id, lookup_service_descriptors_for_interaction,
    parse_pact_from_request_json, to_fully_qualified_name,
};
...
impl ProtobufPactPlugin {
    ...
    fn get_mock_server_results(
        ...
        let ok = results.iter().all(|(_, (req, r))| {
            *req > 0
                && r.iter().all(|(body_result, metadata_result)| {
            ...
            .flat_map(|(path, (req, r))| {
                ...
                if *req == 0 {
                    ...
                    route_results.push(MockServerResult {
                        ...
                        mismatches: r
                            .iter()
                            .flat_map(|(body_result, metadata_result)| {
    ...
    fn compare_contents_impl(
        ...
        let expected_message_type = request
            .expected
            .as_ref()
        ...
        let (message, service) =
            Self::lookup_message_and_service(&interaction_config, &expected_message_type)?;

        let descriptors = Self::lookup_descriptors(plugin_configuration, message_key)?;

        let mut expected_body = request
            .expected
            .as_ref()
            .and_then(|body| body.content.clone().map(Bytes::from))
            .unwrap_or_default();
        let mut actual_body = request
            .actual
            .as_ref()
            .and_then(|body| body.content.clone().map(Bytes::from))
            .unwrap_or_default();
        ...
        let result = if let Some(message_name) = message {
            debug!(
                "Received compare_contents request for message {}",
                message_name
            );
            match_message(
                message_name.as_str(),
            ...
            debug!(
                "Received compareContents request for service {}",
                service_name
            );
            ...
            let expected_content_type = match ContentType::parse(content_type.as_str()) {
                ...
                Err(err) => {
                    return Err(anyhow!(
                        "Expected content type is not set or not valid - {}",
                        err
                    ));
                }
            };
        ...
        match result {
            Ok(result) => match result {
                ...
                BodyMatchResult::BodyMismatches(mismatches) => Ok(CompareContentsResponse {
                    results: mismatches
                        .iter()
                        .map(|(k, v)| {
                            (
    ...
    fn lookup_descriptors(
        ...
        debug!(
            "Pact level configuration keys: {:?}",
            pact_configuration.fields.keys()
        );

        let config_for_interaction = pact_configuration
            .fields
            .iter()
    ...
    fn lookup_message_and_service(
        ...
        if message.is_none() && service.is_none() {
            ...
            Err(anyhow!(
                "Plugin configuration item with key 'message' or 'service' is required"
            ))
        } else {
            Ok((message, service))
        }
    ...
    fn lookup_message_key(
        ...
        if let Some(key) = interaction_config
            .get("descriptorKey")
            .and_then(proto_value_to_string)
        {
            return Ok(key);
        ...
        Err(anyhow!(
            "Plugin configuration item with key 'descriptorKey' is required"
        ))
    }
            ...
            match content_type.attributes.get("message") {
                Some(message_type) => {
                    let (message_descriptor, _) =
                        find_message_descriptor_for_type(message_type, &descriptors)?;
                    ...
                    if body.is_empty() {
                        ...
                        let field_data =
                            decode_message(&mut body, &message_descriptor, &descriptors)?;
                        debug!("message to generate = {:?}", field_data);
                        let generated_message = generate_protobuf_contents(
                            &message_descriptor,
                            &field_data,
                            &content_type,
                            &request.generators,
    ...
    fn verification_preparation_error_response<E: Into<String>>(
        err: E,
    ) -> Response<VerificationPreparationResponse> {
    ...
    fn setup_metadata(
        ...
        for (k, v) in &interaction.request.metadata {
            request_metadata.insert(
                k.clone(),
                proto::MetadataValue {
                    value: Some(proto::metadata_value::Value::NonBinaryValue(
                        to_proto_value(v),
                    )),
                },
            );
        ...
        let service_full_name =
            to_fully_qualified_name(service_desc.name(), file_desc.package()).unwrap_or_default();
        let path =
            build_grpc_route(service_full_name.as_str(), method_desc.name()).unwrap_or_default();
        request_metadata.insert(
            ...
            match entry {
                ...
                KeyAndValueRef::Binary(k, v) => {
                    request_metadata.insert(
                        k.to_string(),
                        proto::MetadataValue {
                            value: Some(proto::metadata_value::Value::BinaryValue(
                                v.to_bytes().unwrap_or_default().to_vec(),
                            )),
                        },
                    );
                }
    ...
    fn apply_generators_to_metadata(
        ...
        let metadata_generators = interaction
            .request
            .generators
            .categories
            ...
            for (key, generator) in &metadata_generators {
                if generator.corresponds_to_mode(&GeneratorTestMode::Provider) {
                    if let Some(header) = key.first_field() {
                        match generator.generate_value(
                            &MessageMetadataValue::default(),
                            &test_context,
                            &vm,
                        ) {
                            Ok(v) => {
                            ...
                            Err(err) => {
                                warn!(
                                    "Failed to generate value for metadata key '{}': {}",
                                    key, err
                                );
                            }
                        }
    ...
    fn configure_interaction_error_response<S: Into<String>>(
        message: S,
...
impl PactPlugin for ProtobufPactPlugin {
    ...
    async fn configure_interaction(
        ...
        let proto_file = match fields.get("pact:proto").and_then(proto_value_to_string) {
            ...
            None => {
                ...
                return Ok(Self::configure_interaction_error_response(
                    "Config item with key 'pact:proto' and path to the proto file is required",
                ));
            }
        };
        ...
        let protoc =
            match setup_protoc(&plugin_config, &self.additional_includes(&plugin_config)).await {
                Ok(protoc) => protoc,
                Err(err) => {
                    error!("Failed to invoke protoc: {}", err);
                    return Ok(Self::configure_interaction_error_response(format!(
        ...
        match process_proto(proto_file, &protoc, &fields).await {
            ...
            Err(err) => {
                ...
                Ok(Self::configure_interaction_error_response(format!(
                    "Failed to process protobuf: {}",
                    err
                )))
            }
        }
    ...
    async fn start_mock_server(
        ...
        let request = request.get_ref();
        let pact =
            match parse_pact_from_request_json(request.pact.as_str(), "grpc:start_mock_server") {
                Ok(pact) => pact,
                Err(err) => {
                    return Ok(Self::start_mock_server_error(format!(
                        "Failed to parse Pact JSON: {}",
                        err
                    )));
                }
            };
        ...
        let plugin_config = match pact.plugin_data.iter().find(|pd| pd.name == "protobuf") {
            None => {
                ...
                return Ok(Self::start_mock_server_error(
                    "Provided Pact file does not have any Protobuf descriptors".to_string(),
                ));
            }
            Some(config) => config.clone(),
        };

        let test_context: HashMap<String, Value> = match request
            .test_context
            .as_ref()
            .map(proto_struct_to_json)
            .unwrap_or_default()
        ...
        match grpc_mock_server
            .start_server(request.host_interface.as_str(), request.port, request.tls)
            .await
        {
            Ok(address) => {
        ...
        if let Some((_, results)) = guard.get(&request.server_key) {
            ...
            Ok(tonic::Response::new(proto::ShutdownMockServerResponse {
                ...
                results: vec![proto::MockServerResult {
                    error: format!(
                        "Did not find any mock server results for a server with ID {}",
                        request.server_key
                    ),
    ...
    async fn prepare_interaction_for_verification(
        ...
        let request = request.get_ref();
        trace!(
            "Got prepare_interaction_for_verification request {:?}",
            request
        );

        let pact = match parse_pact_from_request_json(
            request.pact.as_str(),
            "grpc:prepare_interaction_for_verification",
        ) {
            Ok(pact) => pact,
            Err(err) => {
                return Ok(Self::verification_preparation_error_response(format!(
                    "Failed to parse Pact JSON: {}",
                    err
                )));
            }
        ...
        let interaction = match interaction_by_id {
            Some(interaction) => match interaction.as_v4_sync_message() {
                ...
                None => {
                    return Ok(Self::verification_preparation_error_response(format!(
                        "gRPC interactions must be of type V4 synchronous message, got {}",
                        interaction.type_of()
                    )));
                }
            },
            None => {
                error!(
                    ?key,
                    "Did not find an interaction that matches the given key"
                );
                return Ok(Self::verification_preparation_error_response(format!(
                    "Did not find an interaction that matches the given key '{}'",
                    key
        ...
        let (all_file_desc, service_desc, method_desc, file_desc) =
            match lookup_service_descriptors_for_interaction(&interaction, &pact) {
                Ok(values) => values,
                Err(err) => {
                    return Ok(Self::verification_preparation_error_response(
                        err.to_string(),
                    ));
                }
            };
        ...
        let input_message =
            match find_message_descriptor_for_type(method_desc.input_type(), &all_file_desc) {
        ...
        let test_context = config
            ...
            .collect();
        let decoded_body =
            match decode_message(&mut raw_request_body, &input_message, &all_file_desc) {
                Ok(field_values) => {
        ...
        let mut request_metadata = Self::setup_metadata(
            &interaction,
            &service_desc,
            &method_desc,
            &file_desc,
    ...
    async fn verify_interaction(
        ...
        let pact =
            match parse_pact_from_request_json(request.pact.as_str(), "grpc:verify_interaction") {
                Ok(pact) => pact,
                Err(err) => {
                    return Ok(Self::verify_interaction_error(format!(
                        "Failed to parse Pact JSON: {}",
                        err
                    )));
                }
        ...
        let body = match &request.interaction_data {
            Some(data) => match &data.body {
                Some(b) => match &b.content {
                    Some(data) => OptionalBody::Present(
                        Bytes::from(data.clone()),
                        Some(ContentType::from(b.content_type.clone())),
                        None,
                    ),
            ...
            Ok((result, output)) => {
                let results = result
                    ...
                    .flat_map(|result| match result {
                        VerificationMismatchResult::Mismatches { mismatches, .. } => mismatches
                            .iter()
                            .map(|mismatch| {
                                if let Mismatch::BodyMismatch {
                                    path,
                                    expected,
                                    actual,
                                    mismatch,
                                } = mismatch
                                {
                                    proto::VerificationResultItem {
                                        result: Some(
                                            proto::verification_result_item::Result::Mismatch(
...
fn merge_value(initial: &Value, updated: &Value) -> anyhow::Result<Value> {
    match initial {
        ...
        Value::Object(o) => match updated {
            ...
            _ => bail!(
                "Can not merge config values: {:?} and {:?}",
                initial,
                updated
            ),
        },
...
fn get_interaction_config(
    config: &PluginConfiguration,
) -> anyhow::Result<BTreeMap<String, prost_types::Value>> {
    let interaction_config = config
    ...
    match interaction_config {
        ...
        None => {
            ...
            Err(anyhow!(
                "Plugin configuration for the interaction is required"
            ))
        }
    }
}
...
mod tests {
    ...
    use crate::server::{ProtobufPactPlugin, merge_value};
    ...
    async fn init_plugin_test() {
        ...
        expect!(first.values.get("content-types"))
            .to(be_some().value(&"application/protobuf;application/grpc".to_string()));
        ...
        expect!(second.values.get("content-types"))
            .to(be_some().value(&"application/protobuf;application/grpc".to_string()));
    ...
    async fn configure_interaction_test__with_no_config() {
        ...
        let response = plugin
            ...
            .unwrap();
        let response_message = response.get_ref();
        expect!(&response_message.error).to(be_equal_to(
            "Config item with key 'pact:proto' and path to the proto file is required",
        ));
    }
    ...
    fn ProtobufPactPlugin__additional_includes__with_string_value() {
        ...
        expect!(plugin.additional_includes(&config))
            .to(be_equal_to(vec!["/some/path".to_string()]));
    }
    ...
    fn ProtobufPactPlugin__additional_includes__with_non_string_values() {
        ...
        expect!(plugin.additional_includes(&config))
            .to(be_equal_to(vec!["/path1".to_string(), "200".to_string()]));
    }
    ...
    fn get_mock_server_results_test_with_a_mix_of_mismatches_and_no_mismatches() {
        ...
        let md_mismatch = vec![Mismatch::MetadataMismatch {
    ...
    fn get_mock_server_results_test_with_a_path_with_no_requests() {
        ...
        expect!(ok).to(be_false());
        expect!(results.len()).to(be_equal_to(3));
        let path_1_result = results
            .iter()
            .find(|it| it.path == "Req/Path1")
            .unwrap()
            .clone();
        expect!(path_1_result.error).to(be_equal_to(
            "Did not receive any requests for path 'Req/Path1'",
        ));
    }
    ...
    async fn start_mock_server_returns_an_error_if_the_pact_json_is_invalid() {
        ...
        if let Some(start_mock_server_response::Response::Error(message)) =
    ...
    async fn start_mock_server_returns_an_error_if_the_pact_does_not_have_any_descriptors() {
        ...
        if let Some(start_mock_server_response::Response::Error(message)) =
            &response.get_ref().response
        {
            expect!(message).to(be_equal_to(
                "Provided Pact file does not have any Protobuf descriptors",
            ));
        } else {
            panic!("Was expecting an error message");
        }
    ...
    async fn shutdown_mock_server_returns_an_error_if_the_server_key_was_not_found() {
        ...
        expect!(&error_response.error).to(be_equal_to(
            "Did not find any mock server results for a server with ID 1234abcd",
        ));
    }
    ...
    fn merge_value_test() {
        ...
        expect!(merge_value(&Value::Null, &Value::String("s".to_string())).unwrap())
            .to(be_equal_to(Value::String("s".to_string())));
        expect!(merge_value(&Value::Null, &Value::Bool(true)).unwrap())
            .to(be_equal_to(Value::Bool(true)));
        expect!(merge_value(&Value::Null, &json!(1)).unwrap()).to(be_equal_to(json!(1)));
        expect!(merge_value(&Value::Null, &Value::Array(vec![])).unwrap())
            .to(be_equal_to(Value::Array(vec![])));
        expect!(merge_value(&Value::Null, &Value::Object(Map::default())).unwrap())
            .to(be_equal_to(Value::Object(Map::default())));

        let s = Value::String("x".to_string());
        expect!(merge_value(&s, &Value::Null).unwrap()).to(be_equal_to(s.clone()));
        expect!(merge_value(&s, &Value::String("s".to_string())).unwrap())
            .to(be_equal_to(Value::String("s".to_string())));
        ...
        expect!(merge_value(&s, &Value::Array(vec![])).unwrap())
            .to(be_equal_to(Value::Array(vec![])));
        expect!(merge_value(&s, &Value::Object(Map::default())).unwrap())
            .to(be_equal_to(Value::Object(Map::default())));

        let b = Value::Bool(false);
        expect!(merge_value(&b, &Value::Null).unwrap()).to(be_equal_to(b.clone()));
        expect!(merge_value(&b, &Value::String("s".to_string())).unwrap())
            .to(be_equal_to(Value::String("s".to_string())));
        ...
        expect!(merge_value(&b, &Value::Array(vec![])).unwrap())
            .to(be_equal_to(Value::Array(vec![])));
        expect!(merge_value(&b, &Value::Object(Map::default())).unwrap())
            .to(be_equal_to(Value::Object(Map::default())));

        let n = json!(100.02);
        expect!(merge_value(&n, &Value::Null).unwrap()).to(be_equal_to(n.clone()));
        expect!(merge_value(&n, &Value::String("s".to_string())).unwrap())
            .to(be_equal_to(Value::String("s".to_string())));
        ...
        expect!(merge_value(&n, &Value::Array(vec![])).unwrap())
            .to(be_equal_to(Value::Array(vec![])));
        expect!(merge_value(&n, &Value::Object(Map::default())).unwrap())
            .to(be_equal_to(Value::Object(Map::default())));

        let a = Value::Array(vec![]);
        expect!(merge_value(&a, &Value::Null).unwrap())
            .to(be_equal_to(Value::Array(vec![Value::Null])));
        expect!(merge_value(&a, &Value::String("s".to_string())).unwrap()).to(be_equal_to(
            Value::Array(vec![Value::String("s".to_string())]),
        ));
        expect!(merge_value(&a, &Value::Bool(true)).unwrap())
            .to(be_equal_to(Value::Array(vec![Value::Bool(true)])));
        expect!(merge_value(&a, &json!(1)).unwrap()).to(be_equal_to(Value::Array(vec![json!(1)])));
        expect!(merge_value(&a, &Value::Object(Map::default())).unwrap()).to(be_equal_to(
            Value::Array(vec![Value::Object(Map::default())]),
        ));
        expect!(merge_value(&a, &Value::Array(vec![])).unwrap())
            .to(be_equal_to(Value::Array(vec![])));
        expect!(merge_value(&a, &Value::Array(vec![Value::Null])).unwrap())
            .to(be_equal_to(Value::Array(vec![Value::Null])));
        expect!(merge_value(&Value::Array(vec![Value::Null]), &Value::Array(vec![])).unwrap())
            .to(be_equal_to(Value::Array(vec![Value::Null])));
        expect!(
            merge_value(
        ...
        expect!(merge_value(&m, &Value::Object(Map::default())).unwrap())
            .to(be_equal_to(m.clone()));
        expect!(merge_value(&m, &json!({"test": "ok"})).unwrap())
            .to(be_equal_to(json!({"test": "ok"})));
        expect!(merge_value(&json!({"test": "ok"}), &Value::Object(Map::default())).unwrap())
            .to(be_equal_to(json!({"test": "ok"})));
        ...
        expect!(
            merge_value(
                &json!({"additional": ["ok"]}),
                &json!({"additional": ["not ok"], "other": "value"})
            )
            .unwrap()
        )
        .to(be_equal_to(
            json!({"additional": ["ok", "not ok"], "other": "value"}),
        ));
    }
    ...
    fn lookup_message_key_returns_an_error_when_there_is_no_descriptor_key() {
        expect!(ProtobufPactPlugin::lookup_message_key(&btreemap! {}, &None)).to(be_err());
    }
